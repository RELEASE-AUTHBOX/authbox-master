...
def get_menu_caches_footer(request,caches_name,site_id,active_page,kinds=2,exclude_menu=1,parent_name=''):
	caches_timeout=getattr(settings,_BH,43200);menu=cache.get(f"{caches_name}_{kinds}",version=site_id);menu_class=cache.get(f"{caches_name}_class_{kinds}",version=site_id)
	if menu_class is _C:
...
def get_template_info(site_id):
	subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_r).values(_g)[:1]);template=Template.objects.filter(site__id=site_id,is_frontend=_N).annotate(file_path=subquery_foto)[:1];print(_r,template)
...
def user_init_service(request,agency_id):
	context={};template='smart-admin-2/user_initialize_service.html';context['service_opt']=OptServiceType.choices;service_existing=Service.objects.filter(agency_id=agency_id).order_by('kind');context[_S]=service_existing;context[_BK]=agency_id
	if request.method==_I:
...
def tags_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			else:post=Tags.objects.language(_B).create(name=request.POST.get(_H),status=request.POST.get(_T),site_id=site_id);post.set_current_language(_P);post.name=request.POST.get(_H);post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_l))
...
def slideshow_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			if request.POST.get(_M):obj.photo.clear();Photo.objects.create(content_object=obj,file_path=request.POST.get(_M),description=request.POST.get(_Q))
...
def howitworks_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			post.save();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_Q));messages.info(request,mMsgBox.get(_f,request.POST.get(_F)));return redirect(reverse_lazy(_AY))
...
def howitworks_update(request,uuid):
	context={};context[_J]=_AY;active_page=get_translated_active_page(_A_);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_Y;data=HowItWorks.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);post_photo=post.photo.all()
	if post_photo:post_photo=post_photo.first()
	if request.method==_I:
		form=HowItWorksForm(request.POST,instance=post);photo=PhotoForm(request.POST,instance=post_photo)
		if form.is_valid():
			lang=request.POST.get(_i);obj=data.get();obj.set_current_language(lang);obj.icon=request.POST.get(_R);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_U);obj.content=request.POST.get(_K);obj.is_header_text=form.cleaned_data[_k];order_item=request.POST.get(_B0)
...
def videogallery_ajax(request):
	site_id=get_site_id(request);lst=[];obj=VideoGallery();obj.set_current_language(_P);subquery1=Subquery(VideoGalleryTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model='videogallery').values(_g));lang=obj.get_current_language();obj2=VideoGallery.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(file_path=subquery_foto)
...
def categories_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=Categories.objects.language(_B).create(name=request.POST.get(_H),status=request.POST.get(_T),site_id=site_id);post.set_current_language(_P);post.name=request.POST.get(_H);post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_j))
...
def templateowner_update(request,uuid):
	context={};context[_J]=_Ad;active_page=get_translated_active_page(_B8);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_Y;data=TemplateOwner.objects.filter(uuid=uuid);post=get_object_or_404(data)
	if request.method==_I:
...
def template_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			obj=data.get();tmp=request.POST.get(_BA);obj.name=request.POST.get(_H);obj.rel_path=request.POST.get('rel_path');obj.is_frontend=bool(request.POST.get(_BY))
...
def modellist_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=ModelList.objects.create(name=request.POST.get(_H),description=request.POST.get(_Q),status=request.POST.get(_T));messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_Ae))
...
def modellist_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():obj=data.get();obj.name=request.POST.get(_H);obj.description=request.POST.get(_Q);obj.status=request.POST.get(_T);obj.save();messages.info(request,mMsgBox.get(_Z,request.POST.get(_H)));return redirect(reverse_lazy(_Ae))
