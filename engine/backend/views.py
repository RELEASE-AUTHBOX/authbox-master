...
_BR='Description'
_BQ='select2_update'
_BP='Access From Menu cannot empty!'
_BO='pages/detail/'
_BN='Access From Menu'
_BM='file_exists'
_BL='FOUND SERVICE'
_BK='agency_id'
_BJ='my_active_footer'
_BI='load menu from Cache'
_BH='CACHES_TIMEOUT'
_BG='count'
_BF='%d %b %y'
_BE='end_date='
...
_As='error'
_Ar='/accounts/logout/'
_Aq='Group ID Not found!'
_Ap='load menu from DB'
_Ao='calendar_id'
_An='email'
_Am='payment_notes'
_Al='order_menu'
...
def get_menu_caches(request,caches_name,site_id,active_page,kinds=2,exclude_menu=0,is_visibled=-1):
	print('INSIDE get_menu_caches()');menu_class=_C;print(_Ap);menu_list=[];group_id=0
	if kinds==2:
		...
		if group_id<=2:
			...
			if temp:temp=temp.get().id;model_list=list(ModelListSetting.objects.filter(template_id=temp).values_list('model_list_id',flat=_U))
		else:
		...
		if model_list:menu_list=list(ModelList.objects.filter(id__in=model_list).values_list('menu_id',flat=_U))
	elif kinds==1:
...
def get_menu_caches_footer(request,caches_name,site_id,active_page,kinds=2,exclude_menu=1,parent_name=''):
	...
	if menu_class is _C:
		print(_Ap);menu_list=[];group_id=0
		if kinds==2:
			user_id=request.user.id;obj=User.objects.get(id=user_id);group_id=obj.groups.all()[:1]
			if group_id:group_id=group_id.get().id
		elif kinds==1:
	...
	return{caches_name:menu_footer,_BJ:menu_active}
def get_menu_caches_footer2(request,caches_name,site_id,active_page,kinds=2,exclude_menu=1,parent_order=1):
	...
	if menu:
		for i in menu:
			if not i[_Ae]:parent_name.append(i[_A])
...
def get_template_info(site_id):
	subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_r).values(_i)[:1]);template=Template.objects.filter(site__id=site_id,is_frontend=_U).annotate(file_path=subquery_foto)[:1];print(_r,template)
	if template:return template[0]
	return _C
def get_init_template_id(site_id):
	A='template=';template=Template.objects.filter(site__id=site_id,status=OptStatusPublish.PUBLISHED,is_frontend=_U)[:1];print(A,template)
	if template:template=template.get();print(A,template.id,template.name);return{_A:template.id,_AE:template.name}
	return _C
class IndexView(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		...
		elif self.site_id==-2:print('begin logout');return redirect(_Ar)
	...
	def get_context_data(self,*args,**kwargs):
		context=super(IndexView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_AD);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);agency=get_agency(self.request);context[_N]=agency;service=[]
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_Q]=service;context[_r]=get_template_info(self.site_id);context['template_init']=get_init_template_id(self.site_id);context.update(menu);return context
@transaction.atomic
def user_init_agency(request):
	...
	if user:
		...
		for i in agency:
			if i.is_default:print(_BL);return redirect(f"/dashboard/user/initialize/service/{i.id}")
	else:return redirect(_Ar)
	if request.method==_I:
		name=request.POST.get('agent-name');agency,created=Agency.objects.language(_A).get_or_create(name=name,defaults={'is_default':_U,_AN:'',_AO:''})
		...
		else:context[_As]=f'"{name}" sudah digunakan!'
...
def user_init_agency_ajax(request,agency_id):
	...
	for i in user.agency.filter(is_default=_U):i.is_default=_B;i.save()
	agency=user.agency.filter(id=agency_id)
	if agency:agency=agency.get();agency.is_default=_U;agency.save()
	return JsonResponse({_At:_U},safe=_B)
@transaction.atomic
def user_connect_agency(request):
	...
	if request.method==_I:
		...
		else:context[_As]=f'"{code}" tidak ditemukan!'
	print('context',context);return render(request,template,context)
def user_init_service_ajax(request,agency_id,service_id):
	print('REAL service_id_ajax',agency_id,service_id);mfound=_B;user=User.objects.get(id=request.user.id);agency=user.agency.filter(is_default=_U)[:1]
	if agency:
		agency=agency.get()
		if agency.id==agency_id:print('OKE PARAMETER BENAR');mfound=_U
	if mfound:
		print('clear all is_default service');service=Service.objects.filter(agency_id=agency.id,is_default=_U)
		for i in service:i.is_default=_B;i.save()
		service=Service.objects.filter(id=service_id,agency_id=agency.id)
		if service:service=service.get();service.is_default=_U;service.save()
		return JsonResponse({_At:_U},safe=_B)
	return JsonResponse({_At:_B},safe=_B)
@transaction.atomic
def user_init_service(request,agency_id):
	context={};template='smart-admin-2/user_initialize_service.html';context['service_opt']=OptServiceType.choices;service_existing=Service.objects.filter(agency_id=agency_id).order_by('kind');context[_Q]=service_existing;context[_BK]=agency_id
	if request.method==_I:
		...
		if srv:srv=srv.get();context[_As]=f'Domain "{site.domain}" sudah digunakan oleh service "{srv.get_kind_display()}"! Satu domain hanya dapat digunakan oleh satu service.'
		else:
			srv=Service.objects.create(site_id=site.id,kind=service,agency_id=agency_id,expired_date=tgl_exp,is_active=_U,is_default=_U);template=Template.objects.filter(site=site.id,is_frontend=_U)
			if template:
				for i in template:
					for j in i.site.all():
						if j.id==site.id:i.site.remove(site.id);print(f"DONE remove {site.name} from template {i}")
			template_id=_C;temp=Template.objects.filter(is_frontend=_U)
			for i in temp:
...
def service_change_ajax(request,service_id):
	print('service_id=',service_id);lst=[];temp=Template.objects.filter(is_frontend=_U)
	for i in temp:
		if service_id in i.service_option:res={};res[_A]=i.id;res[_AE]=i.name;lst.append(res)
	print('res',lst);return JsonResponse({_Af:lst},safe=_B)
...
def tags_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_AP]=Truncator(i.name_id).chars(50);res[_AQ]=Truncator(i.name).chars(50);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
...
def tags_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			if tmp:messages.info(request,mMsgBox.get(_BM));context[_G]=TagsForm()
			else:post=Tags.objects.language(_A).create(name=request.POST.get(_H),status=request.POST.get(_R),site_id=site_id);post.set_current_language(_O);post.name=request.POST.get(_H);post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_l))
...
def tags_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.name=request.POST.get(_H);obj.status=request.POST.get(_R);obj.site_id=site_id;obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_H)));return redirect(reverse_lazy(_l))
...
class LogoView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_Q]=service;context[_AR]=_U;active_page=get_translated_active_page(_w);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def logo_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_AF]=Truncator(i.name).chars(50);res['Pos']=i.get_pos_display();res[_m]=i.file_path;res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def logo_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=form.save(commit=_B);post.site_id=site_id;post.save();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S));messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_w))
...
def logo_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S))
			messages.info(request,mMsgBox.get(_Y,request.POST.get(_H)));return redirect(reverse_lazy(_w))
	else:
...
def announcement_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res['Priority']=i.get_priority_display();res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
...
def announcement_update(request,uuid):
	...
	return render(request,template,context)
def announcement_delete(request,uuid):context={};site_id=get_site_id(request);data=Announcement.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_x))
...
def fasilities_ajax(request):
	site_id=get_site_id(request);obj=Fasilities();obj.set_current_language(_O);subquery1=Subquery(FasilitiesTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(FasilitiesTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery3=Subquery(FasilitiesTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_a));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_y).values(_i)[:1]);lang=obj.get_current_language();obj2=Fasilities.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_A9]=i.is_header_text;res[_Au]=i.order_item;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def fasilities_create(request):
	context={};context[_J]=_y;active_page=get_translated_active_page(_y);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
		form=FasilitiesForm(request.POST);photo=PhotoForm(request.POST)
		if form.is_valid():
			post=Fasilities.objects.language(_A).create(title=request.POST.get(_F),sub_title=request.POST.get(_a),content=request.POST.get(_K),is_header_text=form.cleaned_data[_k],status=request.POST.get(_R),site_id=site_id,admin_id=request.user.id);post.set_current_language(_O);post.title=request.POST.get(_F);post.sub_title=request.POST.get(_a);post.content=request.POST.get(_K);post.is_header_text=form.cleaned_data[_k];post.save()
			if request.POST.get(_M):Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S))
...
def fasilities_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_a);obj.content=request.POST.get(_K);obj.is_header_text=form.cleaned_data[_k];obj.status=request.POST.get(_R);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save()
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S))
...
def offers_ajax(request):
	site_id=get_site_id(request);obj=Offers();obj.set_current_language(_O);subquery1=Subquery(OffersTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery3=Subquery(OffersTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_a));subquery2=Subquery(OffersTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_A1).values(_i)[:1]);lang=obj.get_current_language();obj2=Offers.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_A9]=i.is_header_text;res[_Au]=i.order_item;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def offers_create(request):
	context={};context[_J]=_A1;active_page=get_translated_active_page(_A1);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
...
def news_ajax(request):
	site_id=get_site_id(request);obj=News();obj.set_current_language(_O);subquery1=Subquery(NewsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(NewsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_A2).values(_i)[:1]);lang=obj.get_current_language();obj2=News.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
...
def news_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_a);obj.content=request.POST.get(_K);obj.categories_id=request.POST.get(_j);obj.status=request.POST.get(_R);obj.site_id=site_id;obj.admin_id=request.user.id;obj.is_header_text=_U if request.POST.get(_k)=='on'else _B;obj.save();obj.tags.clear();save_tags(request.POST.getlist(_l),obj)
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S))
	...
	return render(request,template,context)
def news_delete(request,uuid):context={};site_id=get_site_id(request);data=News.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_A2))
...
def article_ajax(request):
	site_id=get_site_id(request);obj=Article();obj.set_current_language(_O);subquery1=Subquery(ArticleTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(ArticleTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_A3).values(_i)[:1]);lang=obj.get_current_language();obj2=Article.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_A9]=i.is_header_text;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def article_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=Article.objects.language(_A).create(title=request.POST.get(_F),content=request.POST.get(_K),categories_id=request.POST.get(_j),status=request.POST.get(_R),is_header_text=form.cleaned_data[_k],site_id=site_id,admin_id=request.user.id);post.set_current_language(_O);post.title=request.POST.get(_F);post.content=request.POST.get(_K);post.save();save_tags(request.POST.getlist(_l),post);Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S));messages.info(request,mMsgBox.get(_f,request.POST.get(_F)));return redirect(reverse_lazy(_A3))
...
def article_update(request,uuid):
	...
	if request.method==_I:
		...
		if post_photo:photo=PhotoForm(request.POST,instance=post_photo)
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.content=request.POST.get(_K);obj.categories_id=request.POST.get(_j);obj.status=request.POST.get(_R);obj.is_header_text=form.cleaned_data[_k];obj.site_id=site_id;obj.admin_id=request.user.id;obj.save();obj.tags.clear();save_tags(request.POST.getlist(_l),obj)
			if post_photo:
				if photo.is_valid():
					if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S))
	...
	return render(request,template,context)
def article_delete(request,uuid):context={};site_id=get_site_id(request);data=Article.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_A3))
...
def events_ajax(request):
	site_id=get_site_id(request);obj=Events();obj.set_current_language(_O);subquery1=Subquery(EventsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(EventsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_A4).values(_i)[:1]);lang=obj.get_current_language();obj2=Events.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
...
def events_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S))
			messages.info(request,mMsgBox.get(_Y,request.POST.get(_F)));return redirect(reverse_lazy(_A4))
	else:
...
def slideshow_ajax(request):
	site_id=get_site_id(request);lst=[];obj=SlideShow();obj.set_current_language(_O);subquery1=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery3=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_a));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model='slideshow').values(_i)[:1]);lang=obj.get_current_language();obj2=SlideShow.objects.language(lang).filter(site_id=site_id).distinct().annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto)
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def slideshow_create(request):
	context={};context[_J]=_AS;active_page=get_translated_active_page(_Av);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
...
def slideshow_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_a);obj.content=request.POST.get(_K);obj.status=request.POST.get(_R);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save()
			if request.POST.get(_M):obj.photo.clear();Photo.objects.create(content_object=obj,file_path=request.POST.get(_M),description=request.POST.get(_S))
			else:print('photo not valid')
			messages.info(request,mMsgBox.get(_Y,request.POST.get(_F)));return redirect(reverse_lazy(_AS))
	else:
		messages.info(request,mMsgBox.get(_Z));context[_G]=SlideShowForm(instance=post)
		if post_photo:context[_L]=PhotoForm(instance=post_photo)
		else:context[_L]=PhotoForm()
	return render(request,template,context)
def slideshow_delete(request,uuid):context={};site_id=get_site_id(request);data=SlideShow.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_AS))
...
def dailyalert_ajax(request):
	site_id=get_site_id(request);lst=[];obj=DailyAlert();obj.set_current_language(_O);subquery1=Subquery(DailyAlertTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_AI));lang=obj.get_current_language();obj2=DailyAlert.objects.language(lang).filter(site_id=site_id).annotate(alert_id=subquery1)
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res['Alert (id)']=Truncator(i.alert_id).chars(50);res['Alert (en)']=Truncator(i.alert).chars(50);res[_AJ]=Truncator(i.link).chars(50);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
...
class GreetingView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_Q]=service;context[_AR]=_U;active_page=get_translated_active_page(_A5);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def greeting_ajax(request):
	site_id=get_site_id(request);obj=Greeting();obj.set_current_language(_O);subquery1=Subquery(GreetingTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(GreetingTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_A5).values(_i)[:1]);lang=obj.get_current_language();obj2=Greeting.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
...
def greeting_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.content=request.POST.get(_K);obj.name=request.POST.get(_H);obj.designation=request.POST.get(_Ax);obj.status=request.POST.get(_R);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save()
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S))
...
def pages_ajax(request):
	site_id=get_site_id(request);obj=Pages();obj.set_current_language(_O);subquery1=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery3=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_a));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_A6).values(_i)[:1]);lang=obj.get_current_language();obj2=Pages.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
...
def pages_create(request):
	context={};context[_J]=_A6;active_page=get_translated_active_page(_A6);context[_E]=active_page;context[_Ag]=_BN;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			if menu_id:
				...
				if photo.is_valid():
					if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S))
				obj=Menu.objects.get(id=menu_id);obj.link=_BO+post.slug;obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_F)));return redirect(reverse_lazy(_A6))
			else:
				messages.info(request,mMsgBox.get(_Ai,_BP));context[_G]=PagesForm(request.POST,instance=post)
...
def socialmedia_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res['Kind']=i.get_kind_display();res[_AJ]=Truncator(i.link).chars(70);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def socialmedia_create(request):
	context={};context[_J]=_AU;active_page=get_translated_active_page(_Ay);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
		form=SocialMediaForm(request.POST)
		if form.is_valid():post=form.save(commit=_B);post.site_id=site_id;post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_t)));return redirect(reverse_lazy(_AU))
...
def socialmedia_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():post=form.save(commit=_B);post.site_id=site_id;post.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_t)));return redirect(reverse_lazy(_AU))
	else:
		messages.info(request,mMsgBox.get(_Z))
		if post:context[_G]=SocialMediaForm(instance=post)
	return render(request,template,context)
def socialmedia_delete(request,uuid):context={};site_id=get_site_id(request);data=SocialMedia.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.link;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_AU))
...
def howitworks_ajax(request):
	site_id=get_site_id(request);obj=HowItWorks();subquery=Subquery(HowItWorksTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(HowItWorksTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery3=Subquery(HowItWorksTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_a));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model='howitworks').values(_i)[:1]);lang=obj.get_current_language();obj2=HowItWorks.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res[_p]=Truncator(i.content_id).chars(70);res[_q]=Truncator(i.content).chars(70);res[_AV]=i.icon;res[_A9]=i.is_header_text;res[_Au]=i.order_item;res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def howitworks_create(request):
	context={};context[_J]=_AW;active_page=get_translated_active_page(_Az);print(_E,active_page);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
		...
		if form.is_valid():
			...
			post.save();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S));messages.info(request,mMsgBox.get(_f,request.POST.get(_F)));return redirect(reverse_lazy(_AW))
...
def howitworks_update(request,uuid):
	context={};context[_J]=_AW;active_page=get_translated_active_page(_Az);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_X;data=HowItWorks.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);post_photo=post.photo.all()
	if post_photo:post_photo=post_photo.first()
	if request.method==_I:
		form=HowItWorksForm(request.POST,instance=post);photo=PhotoForm(request.POST,instance=post_photo)
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.icon=request.POST.get(_P);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_a);obj.content=request.POST.get(_K);obj.is_header_text=form.cleaned_data[_k];order_item=request.POST.get(_A_)
			if int(order_item)>0:obj.order_item=order_item
			obj.site_id=site_id;obj.admin_id=request.user.id;obj.save()
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S))
			messages.info(request,mMsgBox.get(_Y,request.POST.get(_F)));return redirect(reverse_lazy(_AW))
	else:
		messages.info(request,mMsgBox.get(_Z));context[_G]=HowItWorksForm(instance=post)
		if post_photo:context[_L]=PhotoForm(instance=post_photo)
		else:context[_L]=PhotoForm()
	return render(request,template,context)
def howitworks_delete(request,uuid):context={};site_id=get_site_id(request);data=HowItWorks.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_AW))
...
def aboutus_ajax(request):
	site_id=get_site_id(request);obj=AboutUs();subquery=Subquery(AboutUsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(AboutUsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_a));subquery3=Subquery(AboutUsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model='aboutus').values(_i)[:1]);lang=obj.get_current_language();obj2=AboutUs.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery).annotate(sub_title_id=subquery2).annotate(content_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(70);res[_o]=Truncator(i.title).chars(70);res[_z]=Truncator(i.sub_title_id).chars(70);res[_A0]=Truncator(i.sub_title).chars(70);res[_p]=Truncator(i.content_id).chars(70);res[_q]=Truncator(i.content).chars(70);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def aboutus_create(request):
	context={};context[_J]=_AX;active_page=get_translated_active_page(_B0);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
...
def aboutus_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_a);obj.content=request.POST.get(_K);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save()
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S))
			messages.info(request,mMsgBox.get(_Y,request.POST.get(_F)));return redirect(reverse_lazy(_AX))
	else:
		messages.info(request,mMsgBox.get(_Z));context[_G]=AboutUsForm(instance=post)
		if post_photo:context[_L]=PhotoForm(instance=post_photo)
		else:context[_L]=PhotoForm()
	return render(request,template,context)
def aboutus_delete(request,uuid):context={};site_id=get_site_id(request);data=AboutUs.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_AX))
...
def testimony_ajax(request):
	site_id=get_site_id(request);obj=Testimony();subquery=Subquery(TestimonyTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_A7).values(_i)[:1]);lang=obj.get_current_language();obj2=Testimony.objects.language(lang).filter(site_id=site_id).annotate(content_id=subquery).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res['Title']=Truncator(i.title).chars(70);res['Sub Title']=Truncator(i.subtitle).chars(70);res[_p]=Truncator(i.content_id).chars(70);res[_q]=Truncator(i.content).chars(70);res[_A9]=i.is_header_text;res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def testimony_create(request):
	context={};context[_J]=_A7;active_page=get_translated_active_page(_A7);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
...
def testimony_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.subtitle=request.POST.get(_AK);obj.content=request.POST.get(_K);obj.site_id=site_id;obj.admin_id=request.user.id;obj.is_header_text=form.cleaned_data[_k];obj.save()
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S))
			messages.info(request,mMsgBox.get(_Y,request.POST.get(_F)));return redirect(reverse_lazy(_A7))
	else:
...
def photogallery_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_p]=Truncator(i.content_id).chars(100);res[_q]=Truncator(i.content).chars(100);res[_m]=i.file_path;res[_A9]=i.is_header_text;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def photogallery_create(request):
	...
	if request.method==_I:
		...
		else:print(_AH);context[_G]=PhotoGalleryForm();context[_L]=PhotoForm()
...
def photogallery_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.status=request.POST.get(_R);obj.content=request.POST.get(_K);obj.site_id=site_id;obj.admin_id=request.user.id;obj.is_header_text=form.cleaned_data[_k];obj.save()
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S))
			messages.info(request,mMsgBox.get(_Y,request.POST.get(_F)));return redirect(reverse_lazy(_AY))
	else:
		messages.info(request,mMsgBox.get(_Z));context[_G]=PhotoGalleryForm(instance=post)
		if post_photo:context[_L]=PhotoForm(instance=post_photo)
		else:context[_L]=PhotoForm()
	return render(request,template,context)
def photogallery_delete(request,uuid):context={};site_id=get_site_id(request);data=PhotoGallery.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_AY))
...
def videogallery_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def videogallery_create(request):
	...
	if request.method==_I:
		...
		else:print(_AH);context[_G]=VideoGalleryForm()
...
def videogallery_update(request,uuid):
	...
	return render(request,template,context)
def videogallery_delete(request,uuid):context={};site_id=get_site_id(request);data=VideoGallery.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_AZ))
class RelatedLinkView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_Q]=service;active_page=get_translated_active_page(_B3);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def relatedlink_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_AP]=Truncator(i.name_id).chars(50);res[_AQ]=Truncator(i.name).chars(50);res[_AJ]=Truncator(i.link).chars(70);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def relatedlink_create(request):
	...
	if request.method==_I:
		...
		else:print(_AH);context[_G]=RelatedLinkForm()
...
def relatedlink_update(request,uuid):
	...
	return render(request,template,context)
def relatedlink_delete(request,uuid):context={};site_id=get_site_id(request);data=RelatedLink.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.name;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_Aa))
class DocumentView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_Q]=service;active_page=get_translated_active_page(_AA);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def document_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_AP]=Truncator(i.name_id).chars(20);res[_AQ]=Truncator(i.name).chars(20);res['content (id)']=Truncator(i.content_id).chars(30);res['content (en)']=Truncator(i.content).chars(30);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def document_create(request):
	context={};context[_J]=_AA;active_page=get_translated_active_page(_AA);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
...
def document_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			obj.status=request.POST.get(_R);obj.categories_id=request.POST.get(_j);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save();obj.size=os.stat(obj.file_path_doc.path).st_size;obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_H)));return redirect(reverse_lazy(_AA))
...
class ApplicationView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super().get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_AL);context[_E]=active_page;menu=get_menu_caches(self.request,_AL,self.site_id,active_page);context.update(menu);return context
def application_ajax(request):
	site_id=get_site_id(request);subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef('ref_template_block__id'),content_type__model='templateblock').values(_i)[:1]);obj2=GlobalSetting.objects.filter(site_id=site_id).annotate(file_path=subquery_foto).order_by(_A_);lst=[]
	for i in obj2:print('ref',i.ref_template_block.id);print('template_block',i.file_path);res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=_C;res[_AF]=i.get_name_display();res['Value']=i.value;res[_BR]=i.ref_template_block.description;res[_m]=i.file_path;res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def application_update(request,uuid):
	C='template_block_model_list_id';B='template_id';A='select2_global_setting';context={};context[_J]=_AL;active_page=get_translated_active_page(_AL);context[_E]=active_page;site_id=get_site_id(request);context[A]='Template Block';template_id=get_template_id(site_id);print(B,template_id);context[B]=template_id;menu=get_menu_caches(request,_AL,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_X;data=GlobalSetting.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);print('ref template block',post.ref_template_block.id);template_block=TemplateBlock.objects.filter(id=post.ref_template_block.id);template_block_model_list_id='0'
	if template_block:
		template_block=template_block.get()
		for i in template_block.model_list.all():print(C,i.id);template_block_model_list_id=i.id
	context[C]=template_block_model_list_id;tmp_template=get_template(site_id,is_frontend=_U);select2_update_global_setting={_A:post.id,_AE:post.value};context['select2_update_global_setting']=select2_update_global_setting
	if request.method==_I:
		...
		if form.is_valid():
			...
			messages.info(request,mMsgBox.get(_Y,label));return redirect(reverse_lazy(_AL))
	else:messages.info(request,mMsgBox.get(_Z));context[_G]=GlobalSettingForm(instance=post)
	return render(request,template,context)
def application_update_ajax(request):
	site_id=get_site_id(request);template=get_template(site_id,is_frontend=_U);search=request.GET.get(_B4);template_id=request.GET.get('t');modellist_id=request.GET.get('ml')
	...
	return JsonResponse({_Af:list(object_list),_B5:{'more':_U}},safe=_B)
	...
	if group_id:
		...
		if menu:
			...
			for i in obj2:
				...
				res={};res[_P]=_C;res[_T]=i[_T];res[_V]=_C;res[_Ak+lang+')']=Truncator(i[_H]).chars(50);res[_Ak+lang2+')']=Menu.objects.language(lang2).get(pk=i[_A]).name;res['Tree']=tmp+Truncator(i[_H]).chars(70);res[_AJ]=Truncator(i[_t]).chars(70);res['Order']=i[_Al];res[_AV]=Truncator(i[_P]).chars(50);res['Visibled']=Truncator(i[_AM]).chars(50);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
	...
	if group_id:
		...
		if menu:
			...
			for i in obj2:
				...
				res={};res[_P]=_C;res[_T]=i[_T];res[_V]=_C;res[_Ak+lang+')']=Truncator(i[_H]).chars(50);res[_Ak+lang2+')']=Menu.objects.language(lang2).get(pk=i[_A]).name;res['Tree']=tmp+Truncator(i[_H]).chars(70);res[_AJ]=Truncator(i[_t]).chars(70);res['Order']=i[_Al];res[_AV]=Truncator(i[_P]).chars(50);res['Visibled']=Truncator(i[_AM]).chars(50);res['Exclude']=Truncator(i[_B6]).chars(50);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def menu_create(request):
	context={};context[_J]=_D;active_page=get_translated_active_page(_D);context[_E]=active_page;context[_Ag]=_BS;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);group_id=get_menu_group(request,site_id);menu_group=MenuGroup.objects.get(id=group_id);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
		form=MenuForm(request.POST)
		if form.is_valid():form_clean=form.cleaned_data;post=Menu.objects.language(_A).create(name=form_clean[_H],parent_id=request.POST.get(_Ah),link=form_clean[_t],order_menu=form_clean[_Al],icon=form_clean[_P],is_visibled=form_clean[_AM],is_external=form_clean[_BT],exclude_menu=form_clean[_B6]);post.menu_group.add(menu_group);post.set_current_language(_O);post.name=form_clean[_H];post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_D))
		else:context[_G]=MenuForm()
	else:messages.info(request,mMsgBox.get(_d));context[_G]=MenuForm()
	return render(request,template,context)
def menu_update(request,uuid):
	context={};context[_J]=_D;active_page=get_translated_active_page(_D);context[_E]=active_page;context[_Ag]=_BS;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_X;data=Menu.objects.filter(uuid=uuid);post=get_object_or_404(data);select2_update={_A:post.parent_id,_AE:post.parent.name if post.parent else _C};context[_BQ]=select2_update
	if request.method==_I:
		form=MenuForm(request.POST,instance=post)
		if form.is_valid():form_clean=form.cleaned_data;lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.name=form_clean[_H];obj.parent_id=request.POST.get(_Ah);obj.link=form_clean[_t];obj.order_menu=form_clean[_Al];obj.icon=form_clean[_P];obj.is_visibled=form_clean[_AM];obj.is_external=form_clean[_BT];obj.exclude_menu=form_clean[_B6];obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_H)));return redirect(reverse_lazy(_D))
...
class AgencyView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_Q]=service;active_page=get_translated_active_page(_N);context[_E]=active_page;context[_AR]=_U;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def agency_ajax(request):
	site_id=get_site_id(request);obj=Agency();obj.set_current_language(_O);service=Service.objects.filter(site_id=site_id).values_list(_N,flat=_U);subquery1=Subquery(AgencyTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_AN));subquery2=Subquery(AgencyTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_AO));subquery3=Subquery(AgencyTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_Am));lang=obj.get_current_language();obj2=Agency.objects.language(lang).filter(id=service[0]).annotate(address_id=subquery1).annotate(notes_id=subquery2).annotate(payment_notes_id=subquery3);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_AF]=Truncator(i.name).chars(50);res['Address (id)']=Truncator(i.address_id).chars(50);res['Address (en)']=Truncator(i.address).chars(50);res[_BU]=Truncator(i.notes_id).chars(50);res[_BV]=Truncator(i.notes).chars(50);res['Payment Notes (id)']=Truncator(i.payment_notes_id).chars(50);res['Payment Notes (en)']=Truncator(i.payment_notes).chars(50);res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
...
class CategoriesView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_Q]=service;active_page=get_translated_active_page(_j);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
...
def categories_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=Categories.objects.language(_A).create(name=request.POST.get(_H),status=request.POST.get(_R),site_id=site_id);post.set_current_language(_O);post.name=request.POST.get(_H);post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_j))
...
def categories_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.name=request.POST.get(_H);obj.status=request.POST.get(_R);obj.site_id=site_id;obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_H)));return redirect(reverse_lazy(_j))
...
class ProductView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_Q]=service;active_page=get_translated_active_page(_A8);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def product_ajax(request):
	site_id=get_site_id(request);obj=Product();obj.set_current_language(_O);subquery1=Subquery(ProductTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_H));subquery2=Subquery(ProductTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery3=Subquery(ProductTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery4=Subquery(ProductTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_a));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_A8).values(_i)[:1]);lang=obj.get_current_language();obj2=Product.objects.language(lang).filter(site_id=site_id).annotate(name_id=subquery1).annotate(title_id=subquery2).annotate(content_id=subquery3).annotate(sub_title_id=subquery4).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_AP]=Truncator(i.name_id).chars(50);res[_AQ]=Truncator(i.name).chars(50);res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res['price']=format_money(i.price,locale='ID');res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_A9]=i.is_header_text;res[_AV]=i.icon;res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
...
class CalendarView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_Q]=service;active_page=get_translated_active_page(_AC);context[_E]=active_page;context[_AR]=_U;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
...
def calendar_ajax(request):
	...
	for i in obj2:
		res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res['calendar Name']=Truncator(i.calendar_id).chars(50)
		if i.file_path_doc:res[A]=Truncator(i.file_path_doc.url).chars(50)
		else:res[A]=_C
		res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def calendar_create(request):
	context={};context[_J]=_AC;active_page=get_translated_active_page(_AC);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
		form=GoogleCalendarForm(request.POST,request.FILES)
		if form.is_valid():post=GoogleCalendar.objects.update_or_create(calendar_id=request.POST.get(_Ao),defaults={_AB:request.FILES.get(_AB),'site_id':site_id});messages.info(request,mMsgBox.get(_f,request.POST.get(_Ao)));return redirect(reverse_lazy(_AC))
	else:messages.info(request,mMsgBox.get(_d));context[_G]=GoogleCalendarForm()
	return render(request,template,context)
def calendar_update(request,uuid):
	context={};context[_J]=_AC;active_page=get_translated_active_page(_AC);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_X;data=GoogleCalendar.objects.filter(uuid=uuid);post=get_object_or_404(data)
	if request.method==_I:
		form=GoogleCalendarForm(request.POST,request.FILES,instance=post)
		if form.is_valid():
			obj=data.get();obj.calendar_id=request.POST.get(_Ao)
			if request.FILES.get(_AB):obj.file_path_doc=request.FILES.get(_AB)
			obj.site_id=site_id;obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_Ao)));return redirect(reverse_lazy(_AC))
	else:messages.info(request,mMsgBox.get(_Z));context[_G]=GoogleCalendarForm(instance=post)
	return render(request,template,context)
...
def menu_lookup_ajax(request):
	A='translations__name';lang=get_active_language_choices()[0];site_id=get_site_id(request);group_id=get_menu_group(request,site_id);search=request.GET.get(_B4)
...
def templateowner_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_AF]=Truncator(i.name).chars(50);res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def templateowner_create(request):
	context={};context[_J]=_Ab;active_page=get_translated_active_page(_B7);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
		form=TemplateOwnerForm(request.POST)
		if form.is_valid():post=TemplateOwner.objects.create(name=request.POST.get(_H));messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_Ab))
	else:messages.info(request,mMsgBox.get(_d));context[_G]=TemplateOwnerForm()
	return render(request,template,context)
def templateowner_update(request,uuid):
	context={};context[_J]=_Ab;active_page=get_translated_active_page(_B7);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_X;data=TemplateOwner.objects.filter(uuid=uuid);post=get_object_or_404(data)
	if request.method==_I:
		form=TemplateOwnerForm(request.POST,instance=post)
		if form.is_valid():obj=data.get();obj.name=request.POST.get(_H);obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_H)));return redirect(reverse_lazy(_Ab))
	else:messages.info(request,mMsgBox.get(_Z));context[_G]=TemplateOwnerForm(instance=post)
	return render(request,template,context)
def templateowner_delete(request,uuid):context={};site_id=get_site_id(request);data=TemplateOwner.objects.filter(uuid=uuid);post=get_object_or_404(data);tmp=post.name;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_Ab))
class TemplateView_(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_Q]=service;active_page=get_translated_active_page(_B8);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def template_ajax(request):
	site_id=get_site_id(request);subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_r).values(_i)[:1]);obj2=Template.objects.filter(is_frontend=_U).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_AF]=Truncator(i.name).chars(50);res['Relative Path']=Truncator(i.rel_path).chars(50);res['Template Owner']=Truncator(i.template_owner).chars(50);res['Frontend']=Truncator(i.is_frontend).chars(50);res[_m]=i.file_path;res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def template_create(request):
	context={};context[_J]=_r;active_page=get_translated_active_page(_B8);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
...
def template_update(request,uuid):
	context={};context[_J]=_r;active_page=get_translated_active_page(_B8);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_X;data=Template.objects.filter(uuid=uuid);post=get_object_or_404(data);post_photo=post.photo.all()
	if post_photo:post_photo=post_photo.first()
...
def modellist_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_AF]=Truncator(i.name).chars(50);res[_BR]=Truncator(i.description).chars(50);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def modellist_create(request):
	context={};context[_J]=_Ac;active_page=get_translated_active_page(_BA);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
		form=ModelListForm(request.POST)
		if form.is_valid():post=ModelList.objects.create(name=request.POST.get(_H),description=request.POST.get(_S),status=request.POST.get(_R));messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_Ac))
	else:messages.info(request,mMsgBox.get(_d));context[_G]=ModelListForm()
	return render(request,template,context)
def modellist_update(request,uuid):
	context={};context[_J]=_Ac;active_page=get_translated_active_page(_BA);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_X;data=ModelList.objects.filter(uuid=uuid);post=get_object_or_404(data)
	if request.method==_I:
		form=ModelListForm(request.POST,instance=post)
		if form.is_valid():obj=data.get();obj.name=request.POST.get(_H);obj.description=request.POST.get(_S);obj.status=request.POST.get(_R);obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_H)));return redirect(reverse_lazy(_Ac))
	else:messages.info(request,mMsgBox.get(_Z));context[_G]=ModelListForm(instance=post)
	return render(request,template,context)
def modellist_delete(request,uuid):context={};site_id=get_site_id(request);data=ModelList.objects.filter(uuid=uuid);post=get_object_or_404(data);tmp=post.name;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_Ac))
def is_have_group(request,*group_names):
	u=User.objects.filter(pk=request.user.id)
	if u:
		u=u.get()
		if bool(u.groups.filter(name__in=group_names))|u.is_superuser:return _U
	return _B
...
def service_ajax(request):
	...
	lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res['Domain']=i.site.domain;res['Service Kind']=i.get_kind_display();res['Agency']=Truncator(i.agency).chars(50);res['Expired Date']=get_natural_datetime(i.expired_date);res['Active']=i.is_active;res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def service_create(request):
	context={};context[_J]=_Q;active_page=get_translated_active_page(_Q);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
...
def service_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():obj=data.get();obj.site_id=request.POST.get(_AG);obj.kind=request.POST.get('kind');obj.agency_id=request.POST.get(_N);obj.is_active=bool(request.POST.get(_BZ));obj.expired_date=request.POST.get(_Ba);obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_H)));return redirect(reverse_lazy(_Q))
	else:messages.info(request,mMsgBox.get(_Z));context[_G]=ServiceForm(instance=post)
	return render(request,template,context)
def service_delete(request,uuid):context={};site_id=get_site_id(request);data=Service.objects.filter(uuid=uuid);post=get_object_or_404(data);tmp=post.name;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_Q))
...
def banner_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res['Priority']=i.get_priority_display();res[_m]=i.file_path;res[_AJ]=Truncator(i.link).chars(70);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def banner_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=form.save(commit=_B);post.site_id=site_id;post.admin_id=request.user.id;post.save();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S));messages.info(request,mMsgBox.get(_f,request.POST.get('position')));return redirect(reverse_lazy(_u))
...
def banner_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_S));print('DOne')
			messages.info(request,mMsgBox.get(_Y,request.POST.get('position')));return redirect(reverse_lazy(_u))
	else:
...
class LocationView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_Q]=service;context[_AR]=_U;active_page=get_translated_active_page(_s);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def location_ajax(request):
	site_id=get_site_id(request);lst=[];obj=Location();obj.set_current_language(_O);subquery1=Subquery(LocationTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(LocationTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_AK));lang=obj.get_current_language();obj2=Location.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(subtitle_id=subquery2)
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.subtitle_id).chars(50);res[_A0]=Truncator(i.subtitle).chars(50);tmp=Truncator(i.embed).chars(100);tmp=tmp.replace('<',' ');tmp=tmp.replace('>',' ');res['Embed']=tmp;res[_A9]=i.is_header_text;res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def location_create(request):
	...
	if request.method==_I:
		...
		else:print(_AH);context[_G]=LocationForm()
...
class WhyUsView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_Q]=service;active_page=get_translated_active_page(_BB);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def whyus_ajax(request):
	site_id=get_site_id(request);obj=WhyUs();obj.set_current_language(_O);subquery=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_S));subquery3=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_a));lang=obj.get_current_language();obj2=WhyUs.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery).annotate(description_id=subquery2).annotate(sub_title_id=subquery3);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title).chars(50);res[_A0]=Truncator(i.sub_title_id).chars(50);res[_BU]=Truncator(i.description_id).chars(100);res[_BV]=Truncator(i.description).chars(100);res[_AV]=Truncator(i.icon).chars(20);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def whyus_create(request):
	context={};context[_J]=_Ad;active_page=get_translated_active_page(_BB);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_c
	if request.method==_I:
		form=WhyUsForm(request.POST)
		if form.is_valid():
			tmp=WhyUs.objects.filter(translations__title=request.POST.get(_F),site_id=site_id)
			if tmp:messages.info(request,mMsgBox.get(_BM));context[_G]=WhyUsForm()
			else:post=WhyUs.objects.language(_A).create(title=request.POST.get(_F),sub_title=request.POST.get(_a),description=request.POST.get(_S),icon=request.POST.get(_P),status=request.POST.get(_R),site_id=site_id,admin_id=request.user.id);post.set_current_language(_O);post.title=request.POST.get(_F);post.sub_title=request.POST.get(_a);post.description=request.POST.get(_S);post.icon=request.POST.get(_P);post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_F)));return redirect(reverse_lazy(_Ad))
	else:messages.info(request,mMsgBox.get(_d));context[_G]=WhyUsForm()
	return render(request,template,context)
def whyus_update(request,uuid):
	context={};context[_J]=_Ad;active_page=get_translated_active_page(_BB);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_X;data=WhyUs.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data)
	if request.method==_I:
		form=WhyUsForm(request.POST,instance=post)
		if form.is_valid():lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_a);obj.description=request.POST.get(_S);obj.icon=request.POST.get(_P);obj.status=request.POST.get(_R);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_F)));return redirect(reverse_lazy(_Ad))
	else:messages.info(request,mMsgBox.get(_Z));context[_G]=WhyUsForm(instance=post)
	return render(request,template,context)
def whyus_delete(request,uuid):context={};site_id=get_site_id(request);data=WhyUs.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_Ad))
...
def user_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_T]=i.uuid;res[_V]=i.updated_at;res['Email']=Truncator(i.email).chars(50);res[_AF]=Truncator(i.name).chars(50);res['Confirm']=i.email_confirmed;res['Site']=i.site.domain;res[_b]=get_natural_datetime(i.updated_at);res[_W]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def user_create(request):
	...
	if request.method==_I:
		...
		else:messages.info(request,mMsgBox.get(_Ai,request.POST.get(_An)));context[_G]=CustomUserCreationForm(request.POST)
	else:messages.info(request,mMsgBox.get(_d));context[_G]=CustomUserCreationForm()
	return render(request,template,context)
def user_update(request,uuid):
	context={};context[_J]=_v;active_page=get_translated_active_page(_v);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_X;data=User.objects.filter(uuid=uuid);post=get_object_or_404(data)
	if request.method==_I:
		form=CustomUserChangeForm(request.POST,instance=post)
		if form.is_valid():obj=data.get();obj.name=request.POST.get(_H);obj.email=request.POST.get(_An);obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_H)));return redirect(reverse_lazy(_v))
...
def get_hitcount_monthly(request):
	B='created__month';A='created__year';lst=[];tgl=datetime.now();site_id=get_site_id(request);content_type_id=ContentType.objects.get(app_label=_BC,model=_AG);content_type_id=content_type_id.id if content_type_id else _C;hitcount_id=HitCount.objects.filter(content_type_id=content_type_id,object_pk=site_id).first();hitcount_id=hitcount_id.id if hitcount_id else _C;start_date=add_months(tgl,-3);start_date=datetime(start_date.year,start_date.month,1,0,0,0);print(_BD,start_date);res=calendar.monthrange(tgl.year,tgl.month);day=res[1];end_date=datetime(tgl.year,tgl.month,day,23,59,59);print(_BE,end_date);hit=Hit.objects.filter(hitcount_id=hitcount_id,created__range=[start_date,end_date]).values(A,B).annotate(count=Count(_A)).order_by(A,B);print(hit);cat=[];val=[]
...
def template_lookup_ajax(request,service_id):
	search=request.GET.get(_B4);template=Template.objects.filter(service_option__contains=service_id,is_frontend=_U,status=OptStatusPublish.PUBLISHED)
...
def template_change(request,template_id):
	site_id=get_site_id(request);template=Template.objects.filter(is_frontend=_U,site__id=site_id)
