...
def get_menu_caches(request,caches_name,site_id,active_page,kinds=2,exclude_menu=0,is_visibled=-1):
	...
	if kinds==2:
		...
		if group_id<=2:
			...
			if temp:temp=temp.get().id;model_list=list(ModelListSetting.objects.filter(template_id=temp).values_list('model_list_id',flat=_N))
		else:
		...
		if model_list:menu_list=list(ModelList.objects.filter(id__in=model_list).values_list('menu_id',flat=_N))
	elif kinds==1:
...
def get_template_info(site_id):
	subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_r).values(_i)[:1]);template=Template.objects.filter(site__id=site_id,is_frontend=_N).annotate(file_path=subquery_foto)[:1];print(_r,template)
...
def user_init_agency(request):
	...
	if request.method==_I:
		name=request.POST.get('agent-name');agency,created=Agency.objects.language(_B).get_or_create(name=name,defaults={'is_default':_N,_AO:'',_AP:''})
...
def user_init_agency_ajax(request,agency_id):
	...
	if agency:agency=agency.get();agency.is_default=_N;agency.save()
	return JsonResponse({_At:_N},safe=_A)
@transaction.atomic
...
def user_init_service_ajax(request,agency_id,service_id):
	print('REAL service_id_ajax',agency_id,service_id);mfound=_A;user=User.objects.get(id=request.user.id);agency=user.agency.filter(is_default=_N)[:1]
	if agency:
		agency=agency.get()
		if agency.id==agency_id:print('OKE PARAMETER BENAR');mfound=_N
	if mfound:
		print('clear all is_default service');service=Service.objects.filter(agency_id=agency.id,is_default=_N)
		for i in service:i.is_default=_A;i.save()
		service=Service.objects.filter(id=service_id,agency_id=agency.id)
		if service:service=service.get();service.is_default=_N;service.save()
		return JsonResponse({_At:_N},safe=_A)
	return JsonResponse({_At:_A},safe=_A)
@transaction.atomic
def user_init_service(request,agency_id):
	...
	if request.method==_I:
		...
		if srv:srv=srv.get();context[_As]=f'Domain "{site.domain}" sudah digunakan oleh service "{srv.get_kind_display()}"! Satu domain hanya dapat digunakan oleh satu service.'
			...
			srv=Service.objects.create(site_id=site.id,kind=service,agency_id=agency_id,expired_date=tgl_exp,is_active=_N,is_default=_N);template=Template.objects.filter(site=site.id,is_frontend=_N)
			if template:
				for i in template:
					for j in i.site.all():
						if j.id==site.id:i.site.remove(site.id);print(f"DONE remove {site.name} from template {i}")
			template_id=_C;temp=Template.objects.filter(is_frontend=_N)
			for i in temp:
...
class TagsView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super(TagsView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_l);context[_E]=active_page;user=User.objects.get(id=self.request.user.id);agency=user.agency.all();context[_O]=agency;service=[]
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
...
def tags_ajax(request):
	site_id=get_site_id(request);obj=Tags();obj.set_current_language(_P);subquery=Subquery(TagsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_H));lang=obj.get_current_language();obj2=Tags.objects.language(lang).filter(site_id=site_id).annotate(name_id=subquery);lst=[]
...
def tags_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			else:post=Tags.objects.language(_B).create(name=request.POST.get(_H),status=request.POST.get(_S),site_id=site_id);post.set_current_language(_P);post.name=request.POST.get(_H);post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_l))
...
class LogoView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;context[_AS]=_N;active_page=get_translated_active_page(_w);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
...
def news_ajax(request):
	site_id=get_site_id(request);obj=News();obj.set_current_language(_P);subquery1=Subquery(NewsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(NewsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A2).values(_i)[:1]);lang=obj.get_current_language();obj2=News.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
...
def article_ajax(request):
	site_id=get_site_id(request);obj=Article();obj.set_current_language(_P);subquery1=Subquery(ArticleTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(ArticleTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A3).values(_i)[:1]);lang=obj.get_current_language();obj2=Article.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_A9]=i.is_header_text;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def article_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=Article.objects.language(_B).create(title=request.POST.get(_F),content=request.POST.get(_K),sub_title=request.POST.get(_U),categories_id=request.POST.get(_j),status=request.POST.get(_S),is_header_text=form.cleaned_data[_k],is_editor_choice=form.cleaned_data[_AH],site_id=site_id,admin_id=request.user.id);post.set_current_language(_P);post.title=request.POST.get(_F);post.sub_title=request.POST.get(_U);post.content=request.POST.get(_K);post.save();save_tags(request.POST.getlist(_l),post);Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_T));set_log_all(site_id,_N);messages.info(request,mMsgBox.get(_f,request.POST.get(_F)));return redirect(reverse_lazy(_A3))
...
def events_ajax(request):
	site_id=get_site_id(request);obj=Events();obj.set_current_language(_P);subquery1=Subquery(EventsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(EventsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A4).values(_i)[:1]);lang=obj.get_current_language();obj2=Events.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
...
def slideshow_ajax(request):
	site_id=get_site_id(request);lst=[];obj=SlideShow();obj.set_current_language(_P);subquery1=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery3=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_U));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model='slideshow').values(_i)[:1]);lang=obj.get_current_language();obj2=SlideShow.objects.language(lang).filter(site_id=site_id).distinct().annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto)
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def slideshow_create(request):
	context={};context[_J]=_AT;active_page=get_translated_active_page(_Av);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def dailyalert_ajax(request):
	site_id=get_site_id(request);lst=[];obj=DailyAlert();obj.set_current_language(_P);subquery1=Subquery(DailyAlertTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_AJ));lang=obj.get_current_language();obj2=DailyAlert.objects.language(lang).filter(site_id=site_id).annotate(alert_id=subquery1)
...
class GreetingView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;context[_AS]=_N;active_page=get_translated_active_page(_A5);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def greeting_ajax(request):
	site_id=get_site_id(request);obj=Greeting();obj.set_current_language(_P);subquery1=Subquery(GreetingTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(GreetingTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A5).values(_i)[:1]);lang=obj.get_current_language();obj2=Greeting.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def greeting_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.content=request.POST.get(_K);obj.name=request.POST.get(_H);obj.designation=request.POST.get(_Ax);obj.status=request.POST.get(_S);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save()
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_T))
...
def pages_ajax(request):
	site_id=get_site_id(request);obj=Pages();obj.set_current_language(_P);subquery1=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery3=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_U));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A6).values(_i)[:1]);lang=obj.get_current_language();obj2=Pages.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def howitworks_create(request):
	context={};context[_J]=_AX;active_page=get_translated_active_page(_Az);print(_E,active_page);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def howitworks_update(request,uuid):
	context={};context[_J]=_AX;active_page=get_translated_active_page(_Az);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_Y;data=HowItWorks.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);post_photo=post.photo.all()
	if post_photo:post_photo=post_photo.first()
	if request.method==_I:
		form=HowItWorksForm(request.POST,instance=post);photo=PhotoForm(request.POST,instance=post_photo)
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.icon=request.POST.get(_Q);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_U);obj.content=request.POST.get(_K);obj.is_header_text=form.cleaned_data[_k];order_item=request.POST.get(_A_)
...
def aboutus_create(request):
	context={};context[_J]=_AY;active_page=get_translated_active_page(_B0);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def videogallery_ajax(request):
	site_id=get_site_id(request);lst=[];obj=VideoGallery();obj.set_current_language(_P);subquery1=Subquery(VideoGalleryTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model='videogallery').values(_i));lang=obj.get_current_language();obj2=VideoGallery.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(file_path=subquery_foto)
...
def relatedlink_create(request):
	context={};context[_J]=_Ab;active_page=get_translated_active_page(_B3);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
		form=RelatedLinkForm(request.POST)
		if form.is_valid():post=RelatedLink.objects.language(_B).create(name=request.POST.get(_H),link=request.POST.get(_t),status=request.POST.get(_S),site_id=site_id,admin_id=request.user.id);post.set_current_language(_P);post.name=request.POST.get(_H);post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_Ab))
...
def application_update_ajax(request):
	site_id=get_site_id(request);template=get_template(site_id,is_frontend=_N);search=request.GET.get(_B4);template_id=request.GET.get('t');modellist_id=request.GET.get('ml')
	...
	return JsonResponse({_Ag:list(object_list),_B5:{'more':_N}},safe=_A)
...
class MenuView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super(MenuView,self).get_context_data(*(args),**kwargs);context[_J]=_D;active_page=get_translated_active_page(_D);context[_E]=active_page;agency=get_agency(self.request);context[_O]=agency;service=[]
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
...
class AgencyView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;active_page=get_translated_active_page(_O);context[_E]=active_page;context[_AS]=_N;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def agency_ajax(request):
	site_id=get_site_id(request);obj=Agency();obj.set_current_language(_P);service=Service.objects.filter(site_id=site_id).values_list(_O,flat=_N);subquery1=Subquery(AgencyTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_AO));subquery2=Subquery(AgencyTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_AP));subquery3=Subquery(AgencyTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_An));lang=obj.get_current_language();obj2=Agency.objects.language(lang).filter(id=service[0]).annotate(address_id=subquery1).annotate(notes_id=subquery2).annotate(payment_notes_id=subquery3);lst=[]
...
def agency_update(request,uuid):
	...
	return render(request,template,context)
def agency_delete(request,uuid):context={};site_id=get_site_id(request);data=Agency.objects.filter(uuid=uuid);post=get_object_or_404(data);tmp=post.name;post.delete();messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_O))
class CategoriesView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super(CategoriesView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_j);agency=get_agency(self.request);context[_O]=agency;service=[]
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
...
def categories_ajax(request):
	site_id=get_site_id(request);obj=Categories();obj.set_current_language(_P);subquery1=Subquery(CategoriesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_H));lang=obj.get_current_language();obj2=Categories.objects.language(lang).filter(site_id=site_id).annotate(name_id=subquery1);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_AQ]=Truncator(i.name_id).chars(50);res[_AR]=Truncator(i.name).chars(50);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def categories_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=Categories.objects.language(_B).create(name=request.POST.get(_H),status=request.POST.get(_S),site_id=site_id);post.set_current_language(_P);post.name=request.POST.get(_H);post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_j))
...
class CalendarView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;active_page=get_translated_active_page(_AC);context[_E]=active_page;context[_AS]=_N;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
...
def templateowner_create(request):
	context={};context[_J]=_Ac;active_page=get_translated_active_page(_B7);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def templateowner_update(request,uuid):
	context={};context[_J]=_Ac;active_page=get_translated_active_page(_B7);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_Y;data=TemplateOwner.objects.filter(uuid=uuid);post=get_object_or_404(data)
	if request.method==_I:
...
def template_ajax(request):
	site_id=get_site_id(request);subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_r).values(_i)[:1]);obj2=Template.objects.filter(is_frontend=_N).annotate(file_path=subquery_foto);lst=[]
...
def template_create(request):
	context={};context[_J]=_r;active_page=get_translated_active_page(_B8);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def template_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			obj=data.get();tmp=request.POST.get(_B9);obj.name=request.POST.get(_H);obj.rel_path=request.POST.get('rel_path');obj.is_frontend=bool(request.POST.get(_BZ))
...
class ModelListView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;active_page=get_translated_active_page(_BA);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
...
def modellist_create(request):
	context={};context[_J]=_Ad;active_page=get_translated_active_page(_BA);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def is_have_group(request,*group_names):
	...
	if u:
		u=u.get()
		if bool(u.groups.filter(name__in=group_names))|u.is_superuser:return _N
	return _A
...
class LocationView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;context[_AS]=_N;active_page=get_translated_active_page(_s);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def location_ajax(request):
	site_id=get_site_id(request);lst=[];obj=Location();obj.set_current_language(_P);subquery1=Subquery(LocationTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(LocationTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_AL));lang=obj.get_current_language();obj2=Location.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(subtitle_id=subquery2)
...
def whyus_ajax(request):
	site_id=get_site_id(request);obj=WhyUs();obj.set_current_language(_P);subquery=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_T));subquery3=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_U));lang=obj.get_current_language();obj2=WhyUs.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery).annotate(description_id=subquery2).annotate(sub_title_id=subquery3);lst=[]
...
def template_lookup_ajax(request,service_id):
	search=request.GET.get(_B4);template=Template.objects.filter(service_option__contains=service_id,is_frontend=_N,status=OptStatusPublish.PUBLISHED)
...
def template_change(request,template_id):
	site_id=get_site_id(request);template=Template.objects.filter(is_frontend=_N,site__id=site_id)
	...
	return JsonResponse('False',safe=_A)
def set_log_all(site_id,is_need_refresh):set_log(site_id,'Auto Headline',_N);set_log(site_id,'Article Notes',_N);set_log(site_id,'Flash News',_N);set_log(site_id,'Auto Popular',_N);set_log(site_id,'Related News',_N);set_log(site_id,'Auto Headline Editor Choice',_N)
