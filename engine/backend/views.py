...
def application_ajax(request):
	site_id=get_site_id(request);subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef('ref_template_block__id'),content_type__model='templateblock').values(_h)[:1]);obj2=GlobalSetting.objects.filter(site_id=site_id).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:print('ref',i.ref_template_block.id);print('template_block',i.file_path);res={};res[_P]=_C;res[_S]=i.uuid;res[_T]=i.updated_at;res[_AC]=i.get_name_display();res['Value']=i.value;res[_k]=i.file_path;res[_Z]=get_natural_datetime(i.updated_at);res[_U]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
