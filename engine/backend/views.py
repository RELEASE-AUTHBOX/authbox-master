...
def get_menu_caches(request,caches_name,site_id,active_page,kinds=2,exclude_menu=0,is_visibled=-1):
	...
	if kinds==2:
		...
		if group_id<=2:
			...
			if temp:temp=temp.get().id;model_list=list(ModelListSetting.objects.filter(template_id=temp).values_list('model_list_id',flat=_P))
		else:
		...
		if model_list:menu_list=list(ModelList.objects.filter(id__in=model_list).values_list('menu_id',flat=_P))
	elif kinds==1:
...
def get_menu_caches_footer2(request,caches_name,site_id,active_page,kinds=2,exclude_menu=1,parent_order=1):
	...
	if menu:
		for i in menu:
			if not i[_Af]:parent_name.append(i[_B])
...
def get_template_info(site_id):
	subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_r).values(_i)[:1]);template=Template.objects.filter(site__id=site_id,is_frontend=_P).annotate(file_path=subquery_foto)[:1];
	if template:return template[0]
	return _C
def get_init_template_id(site_id):
	A='template=';template=Template.objects.filter(site__id=site_id,status=OptStatusPublish.PUBLISHED,is_frontend=_P)[:1];
	if template:template=template.get();return{_B:template.id,_AE:template.name}
	return _C
class IndexView(TemplateView):
	site_id=_C
	def get(self,request,*args,**kwargs):
		self.site_id=get_site_id(request);
		if self.site_id==-1:return redirect(reverse_lazy(_AD))
		...
		else:do_init_data(self.site_id);template=get_template(self.site_id,is_frontend=_A);self.template_name=template+'index.html'
...
def user_init_agency(request):
	...
	if request.method==_I:
		name=request.POST.get('agent-name');agency,created=Agency.objects.language(_B).get_or_create(name=name,defaults={'is_default':_P,_AO:'',_AP:''})
		if created:agency.set_current_language(_O);agency.address='';agency.notes='';agency.save()
		if created:user.agency.add(agency);return redirect(reverse_lazy(_AD))
...
def user_init_agency_ajax(request,agency_id):
	...
	for i in user.agency.filter(is_default=_P):i.is_default=_A;i.save()
	agency=user.agency.filter(id=agency_id)
	if agency:agency=agency.get();agency.is_default=_P;agency.save()
	return JsonResponse({_Au:_P},safe=_A)
@transaction.atomic
...
def user_init_service_ajax(request,agency_id,service_id):
	mfound=_A;user=User.objects.get(id=request.user.id);agency=user.agency.filter(is_default=_P)[:1]
	if agency:
		agency=agency.get()
		if agency.id==agency_id:mfound=_P
	if mfound:
		service=Service.objects.filter(agency_id=agency.id,is_default=_P)
		for i in service:i.is_default=_A;i.save()
		service=Service.objects.filter(id=service_id,agency_id=agency.id)
		if service:service=service.get();service.is_default=_P;service.save()
		return JsonResponse({_Au:_P},safe=_A)
	return JsonResponse({_Au:_A},safe=_A)
@transaction.atomic
def user_init_service(request,agency_id):
	...
	if request.method==_I:
		...
		if hostname.find('localhost')<0 and hostname.find('127.0.0.1')<0:
			created=_A;mcount=1;tmp=subdomain;site=_C
			while not created:
		...
		if srv:srv=srv.get();context[_At]=f'Domain "{site.domain}" sudah digunakan oleh service "{srv.get_kind_display()}"! Satu domain hanya dapat digunakan oleh satu service.'
			...
			srv=Service.objects.create(site_id=site.id,kind=service,agency_id=agency_id,expired_date=tgl_exp,is_active=_P,is_default=_P);template=Template.objects.filter(site=site.id,is_frontend=_P)
			if template:
				for i in template:
					for j in i.site.all():
						if j.id==site.id:i.site.remove(site.id);
			template_id=_C;temp=Template.objects.filter(is_frontend=_P)
			for i in temp:
...
def service_change_ajax(request,service_id):
	lst=[];temp=Template.objects.filter(is_frontend=_P)
	for i in temp:
		if service_id in i.service_option:res={};res[_B]=i.id;res[_AE]=i.name;lst.append(res)
	return JsonResponse({_Ag:lst},safe=_A)
...
def tags_ajax(request):
	...
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_AQ]=Truncator(i.name_id).chars(50);res[_AR]=Truncator(i.name).chars(50);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
...
def tags_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			else:post=Tags.objects.language(_B).create(name=request.POST.get(_H),status=request.POST.get(_S),site_id=site_id);post.set_current_language(_O);post.name=request.POST.get(_H);post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_l))
...
class LogoView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;context[_AS]=_P;active_page=get_translated_active_page(_w);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def logo_ajax(request):
	site_id=get_site_id(request);subquery=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_w).values(_i)[:1]);obj2=Logo.objects.filter(site_id=site_id).distinct().annotate(file_path=subquery);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_AF]=Truncator(i.name).chars(50);res['Pos']=i.get_pos_display();res[_m]=i.file_path;res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def logo_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=form.save(commit=_A);post.site_id=site_id;post.save();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_T));messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_w))
...
def announcement_ajax(request):
	site_id=get_site_id(request);obj=Announcement();obj.set_current_language(_O);subquery1=Subquery(AnnouncementTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(AnnouncementTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_x).values(_i)[:1]);lang=obj.get_current_language();obj2=Announcement.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res['Priority']=i.get_priority_display();res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def announcement_create(request):
	context={};context[_J]=_x;active_page=get_translated_active_page(_x);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
		form=AnnouncementForm(request.POST,site_id=site_id);photo=PhotoForm(request.POST)
		if form.is_valid():post=Announcement.objects.language(_B).create(title=request.POST.get(_F),content=request.POST.get(_K),sub_title=request.POST.get(_U),categories_id=request.POST.get(_j),priority=request.POST.get('priority'),status=request.POST.get(_S),is_editor_choice=form.cleaned_data[_AH],site_id=site_id,admin_id=request.user.id);post.set_current_language(_O);post.title=request.POST.get(_F);post.sub_title=request.POST.get(_U);post.content=request.POST.get(_K);post.save();save_tags(request.POST.getlist(_l),post);Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_T));set_log_all(site_id,_P);messages.info(request,mMsgBox.get(_f,request.POST.get(_F)));return redirect(reverse_lazy(_x))
...
def announcement_update(request,uuid):
	...
	return render(request,template,context)
def announcement_delete(request,uuid):context={};site_id=get_site_id(request);data=Announcement.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();set_log_all(site_id,_P);messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_x))
class FasilitiesView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;active_page=get_translated_active_page(_y);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def fasilities_ajax(request):
	site_id=get_site_id(request);obj=Fasilities();obj.set_current_language(_O);subquery1=Subquery(FasilitiesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(FasilitiesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery3=Subquery(FasilitiesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_U));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_y).values(_i)[:1]);lang=obj.get_current_language();obj2=Fasilities.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_A9]=i.is_header_text;res[_Av]=i.order_item;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def offers_ajax(request):
	site_id=get_site_id(request);obj=Offers();obj.set_current_language(_O);subquery1=Subquery(OffersTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery3=Subquery(OffersTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_U));subquery2=Subquery(OffersTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A1).values(_i)[:1]);lang=obj.get_current_language();obj2=Offers.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_A9]=i.is_header_text;res[_Av]=i.order_item;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def news_ajax(request):
	site_id=get_site_id(request);obj=News();obj.set_current_language(_O);subquery1=Subquery(NewsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(NewsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A2).values(_i)[:1]);lang=obj.get_current_language();obj2=News.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
...
def news_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=News.objects.language(_B).create(title=request.POST.get(_F),sub_title=request.POST.get(_U),content=request.POST.get(_K),categories_id=request.POST.get(_j),status=request.POST.get(_S),site_id=site_id,admin_id=request.user.id,is_header_text=_P if request.POST.get(_k)=='on'else _A,is_editor_choice=_P if request.POST.get(_AH)=='on'else _A);post.set_current_language(_O);post.title=request.POST.get(_F);post.sub_title=request.POST.get(_U);post.content=request.POST.get(_K);post.save();save_tags(request.POST.getlist(_l),post);Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_T));set_log_all(site_id,_P);messages.info(request,mMsgBox.get(_f,request.POST.get(_F)));return redirect(reverse_lazy(_A2))
...
def news_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_U);obj.content=request.POST.get(_K);obj.categories_id=request.POST.get(_j);obj.status=request.POST.get(_S);obj.site_id=site_id;obj.admin_id=request.user.id;obj.is_header_text=_P if request.POST.get(_k)=='on'else _A;obj.is_editor_choice=_P if request.POST.get(_AH)=='on'else _A;obj.save();obj.tags.clear();save_tags(request.POST.getlist(_l),obj)
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_T))
			set_log_all(site_id,_P);messages.info(request,mMsgBox.get(_Z,request.POST.get(_F)));return redirect(reverse_lazy(_A2))
	else:
		messages.info(request,mMsgBox.get(_a));context[_G]=NewsForm(instance=post,site_id=site_id)
		if post_photo:context[_L]=PhotoForm(instance=post_photo)
		else:context[_L]=PhotoForm()
	return render(request,template,context)
def news_delete(request,uuid):context={};site_id=get_site_id(request);data=News.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();set_log_all(site_id,_P);messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_A2))
...
def article_ajax(request):
	site_id=get_site_id(request);obj=Article();obj.set_current_language(_O);subquery1=Subquery(ArticleTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(ArticleTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A3).values(_i)[:1]);lang=obj.get_current_language();obj2=Article.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_A9]=i.is_header_text;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def article_create(request):
	context={};context[_J]=_A3;active_page=get_translated_active_page(_A3);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
		form=ArticleForm(request.POST,site_id=site_id);photo=PhotoForm(request.POST)
		if form.is_valid():post=Article.objects.language(_B).create(title=request.POST.get(_F),content=request.POST.get(_K),sub_title=request.POST.get(_U),categories_id=request.POST.get(_j),status=request.POST.get(_S),is_header_text=form.cleaned_data[_k],is_editor_choice=form.cleaned_data[_AH],site_id=site_id,admin_id=request.user.id);post.set_current_language(_O);post.title=request.POST.get(_F);post.sub_title=request.POST.get(_U);post.content=request.POST.get(_K);post.save();save_tags(request.POST.getlist(_l),post);Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_T));set_log_all(site_id,_P);messages.info(request,mMsgBox.get(_f,request.POST.get(_F)));return redirect(reverse_lazy(_A3))
...
def article_update(request,uuid):
	...
	if request.method==_I:
		...
		if post_photo:photo=PhotoForm(request.POST,instance=post_photo)
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.content=request.POST.get(_K);obj.sub_title=request.POST.get(_U);obj.categories_id=request.POST.get(_j);obj.status=request.POST.get(_S);obj.is_header_text=form.cleaned_data[_k];obj.is_editor_choice=form.cleaned_data[_AH];obj.site_id=site_id;obj.admin_id=request.user.id;obj.save();obj.tags.clear();save_tags(request.POST.getlist(_l),obj)
			if post_photo:
				if photo.is_valid():
					if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_T))
			set_log_all(site_id,_P);messages.info(request,mMsgBox.get(_Z,request.POST.get(_F)));return redirect(reverse_lazy(_A3))
	else:
		messages.info(request,mMsgBox.get(_a));context[_G]=ArticleForm(instance=post,site_id=site_id)
		if post_photo:context[_L]=PhotoForm(instance=post_photo)
		else:context[_L]=PhotoForm()
	return render(request,template,context)
def article_delete(request,uuid):context={};site_id=get_site_id(request);data=Article.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();set_log_all(site_id,_P);messages.info(request,mMsgBox.get(_e,tmp));return redirect(reverse_lazy(_A3))
...
def events_ajax(request):
	site_id=get_site_id(request);obj=Events();obj.set_current_language(_O);subquery1=Subquery(EventsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(EventsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A4).values(_i)[:1]);lang=obj.get_current_language();obj2=Events.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
...
def slideshow_ajax(request):
	site_id=get_site_id(request);lst=[];obj=SlideShow();obj.set_current_language(_O);subquery1=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery3=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_U));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model='slideshow').values(_i)[:1]);lang=obj.get_current_language();obj2=SlideShow.objects.language(lang).filter(site_id=site_id).distinct().annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto)
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def slideshow_create(request):
	context={};context[_J]=_AT;active_page=get_translated_active_page(_Aw);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def dailyalert_ajax(request):
	site_id=get_site_id(request);lst=[];obj=DailyAlert();obj.set_current_language(_O);subquery1=Subquery(DailyAlertTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_AJ));lang=obj.get_current_language();obj2=DailyAlert.objects.language(lang).filter(site_id=site_id).annotate(alert_id=subquery1)
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res['Alert (id)']=Truncator(i.alert_id).chars(50);res['Alert (en)']=Truncator(i.alert).chars(50);res[_AK]=Truncator(i.link).chars(50);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
class GreetingView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;context[_AS]=_P;active_page=get_translated_active_page(_A5);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def greeting_ajax(request):
	site_id=get_site_id(request);obj=Greeting();obj.set_current_language(_O);subquery1=Subquery(GreetingTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(GreetingTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A5).values(_i)[:1]);lang=obj.get_current_language();obj2=Greeting.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def pages_ajax(request):
	site_id=get_site_id(request);obj=Pages();obj.set_current_language(_O);subquery1=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery3=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_U));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A6).values(_i)[:1]);lang=obj.get_current_language();obj2=Pages.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def pages_create(request):
	context={};context[_J]=_A6;active_page=get_translated_active_page(_A6);context[_E]=active_page;context[_Ah]=_BO;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def pages_update(request,uuid):
	context={};context[_J]=_A6;active_page=get_translated_active_page(_A6);context[_E]=active_page;context[_Ah]=_BO;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_Y;data=Pages.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);post_photo=post.photo.all()
	if post_photo:post_photo=post_photo.first()
	select2_update={_B:post.menu_id,_AE:post.menu.name if post.menu else _C};context[_BR]=select2_update
	if request.method==_I:
		...
		if form.is_valid():
			...
			if menu_id:
				...
				if photo.is_valid():
					if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_T))
				obj=Menu.objects.get(id=menu_id);obj.link=_BP+post.slug;obj.save();messages.info(request,mMsgBox.get(_Z,request.POST.get(_F)));return redirect(reverse_lazy(_A6))
			else:
...
def socialmedia_ajax(request):
	...
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res['Kind']=i.get_kind_display();res[_AK]=Truncator(i.link).chars(70);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def socialmedia_create(request):
	context={};context[_J]=_AV;active_page=get_translated_active_page(_Az);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
		form=SocialMediaForm(request.POST)
		if form.is_valid():post=form.save(commit=_A);post.site_id=site_id;post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_t)));return redirect(reverse_lazy(_AV))
...
def socialmedia_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():post=form.save(commit=_A);post.site_id=site_id;post.save();messages.info(request,mMsgBox.get(_Z,request.POST.get(_t)));return redirect(reverse_lazy(_AV))
	else:
		messages.info(request,mMsgBox.get(_a))
...
def howitworks_ajax(request):
	site_id=get_site_id(request);obj=HowItWorks();subquery=Subquery(HowItWorksTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(HowItWorksTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery3=Subquery(HowItWorksTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_U));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model='howitworks').values(_i)[:1]);lang=obj.get_current_language();obj2=HowItWorks.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res[_p]=Truncator(i.content_id).chars(70);res[_q]=Truncator(i.content).chars(70);res[_AW]=i.icon;res[_A9]=i.is_header_text;res[_Av]=i.order_item;res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def howitworks_create(request):
	context={};context[_J]=_AX;active_page=get_translated_active_page(_A_);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
		form=HowItWorksForm(request.POST);photo=PhotoForm(request.POST)
		if form.is_valid():
			post=HowItWorks.objects.language(_B).create(icon=request.POST.get(_Q),title=request.POST.get(_F),sub_title=request.POST.get(_U),content=request.POST.get(_K),is_header_text=form.cleaned_data[_k],status=request.POST.get(_S),site_id=site_id,admin_id=request.user.id);post.set_current_language(_O);post.title=request.POST.get(_F);post.sub_title=request.POST.get(_U);post.content=request.POST.get(_K);order_item=request.POST.get(_B0)
...
def howitworks_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_h);obj=data.get();obj.set_current_language(lang);obj.icon=request.POST.get(_Q);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_U);obj.content=request.POST.get(_K);obj.is_header_text=form.cleaned_data[_k];order_item=request.POST.get(_B0)
...
def aboutus_ajax(request):
	site_id=get_site_id(request);obj=AboutUs();subquery=Subquery(AboutUsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(AboutUsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_U));subquery3=Subquery(AboutUsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model='aboutus').values(_i)[:1]);lang=obj.get_current_language();obj2=AboutUs.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery).annotate(sub_title_id=subquery2).annotate(content_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(70);res[_o]=Truncator(i.title).chars(70);res[_z]=Truncator(i.sub_title_id).chars(70);res[_A0]=Truncator(i.sub_title).chars(70);res[_p]=Truncator(i.content_id).chars(70);res[_q]=Truncator(i.content).chars(70);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def aboutus_create(request):
	context={};context[_J]=_AY;active_page=get_translated_active_page(_B1);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def testimony_ajax(request):
	site_id=get_site_id(request);obj=Testimony();subquery=Subquery(TestimonyTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A7).values(_i)[:1]);lang=obj.get_current_language();obj2=Testimony.objects.language(lang).filter(site_id=site_id).annotate(content_id=subquery).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res['Title']=Truncator(i.title).chars(70);res['Sub Title']=Truncator(i.subtitle).chars(70);res[_p]=Truncator(i.content_id).chars(70);res[_q]=Truncator(i.content).chars(70);res[_A9]=i.is_header_text;res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def photogallery_ajax(request):
	site_id=get_site_id(request);lst=[];obj=PhotoGallery();obj.set_current_language(_O);subquery1=Subquery(PhotoGalleryTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(PhotoGalleryTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model='photogallery').values(_i)[:1]);lang=obj.get_current_language();obj2=PhotoGallery.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto)
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_p]=Truncator(i.content_id).chars(100);res[_q]=Truncator(i.content).chars(100);res[_m]=i.file_path;res[_A9]=i.is_header_text;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def videogallery_ajax(request):
	site_id=get_site_id(request);lst=[];obj=VideoGallery();obj.set_current_language(_O);subquery1=Subquery(VideoGalleryTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model='videogallery').values(_i));lang=obj.get_current_language();obj2=VideoGallery.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(file_path=subquery_foto)
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def relatedlink_ajax(request):
	...
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_AQ]=Truncator(i.name_id).chars(50);res[_AR]=Truncator(i.name).chars(50);res[_AK]=Truncator(i.link).chars(70);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
class DocumentView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;active_page=get_translated_active_page(_AA);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def document_ajax(request):
	...
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_AQ]=Truncator(i.name_id).chars(20);res[_AR]=Truncator(i.name).chars(20);res['content (id)']=Truncator(i.content_id).chars(30);res['content (en)']=Truncator(i.content).chars(30);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def document_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			obj.status=request.POST.get(_S);obj.categories_id=request.POST.get(_j);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save();obj.size=os.stat(obj.file_path_doc.path).st_size;obj.save();messages.info(request,mMsgBox.get(_Z,request.POST.get(_H)));return redirect(reverse_lazy(_AA))
...
def application_update_ajax(request):
	site_id=get_site_id(request);template=get_template(site_id,is_frontend=_P);search=request.GET.get(_B5);template_id=request.GET.get('t');modellist_id=request.GET.get('ml')
	...
	return JsonResponse({_Ag:list(object_list),_B6:{'more':_P}},safe=_A)
	...
	if group_id:
		...
		if menu:
			...
			for i in obj2:
				...
				res={};res[_Q]=_C;res[_V]=i[_V];res[_W]=_C;res[_Al+lang+')']=Truncator(i[_H]).chars(50);res[_Al+lang2+')']=Menu.objects.language(lang2).get(pk=i[_B]).name;res['Tree']=tmp+Truncator(i[_H]).chars(70);res[_AK]=Truncator(i[_t]).chars(70);res['Order']=i[_Am];res[_AW]=Truncator(i[_Q]).chars(50);res['Visibled']=Truncator(i[_AN]).chars(50);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
	...
	if group_id:
		...
		if menu:
			...
			for i in obj2:
				...
				res={};res[_Q]=_C;res[_V]=i[_V];res[_W]=_C;res[_Al+lang+')']=Truncator(i[_H]).chars(50);res[_Al+lang2+')']=Menu.objects.language(lang2).get(pk=i[_B]).name;res['Tree']=tmp+Truncator(i[_H]).chars(70);res[_AK]=Truncator(i[_t]).chars(70);res['Order']=i[_Am];res[_AW]=Truncator(i[_Q]).chars(50);res['Visibled']=Truncator(i[_AN]).chars(50);res['Exclude']=Truncator(i[_B7]).chars(50);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
class AgencyView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;active_page=get_translated_active_page(_N);context[_E]=active_page;context[_AS]=_P;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def agency_ajax(request):
	site_id=get_site_id(request);obj=Agency();obj.set_current_language(_O);service=Service.objects.filter(site_id=site_id).values_list(_N,flat=_P);subquery1=Subquery(AgencyTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_AO));subquery2=Subquery(AgencyTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_AP));subquery3=Subquery(AgencyTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_An));lang=obj.get_current_language();obj2=Agency.objects.language(lang).filter(id=service[0]).annotate(address_id=subquery1).annotate(notes_id=subquery2).annotate(payment_notes_id=subquery3);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_AF]=Truncator(i.name).chars(50);res['Address (id)']=Truncator(i.address_id).chars(50);res['Address (en)']=Truncator(i.address).chars(50);res[_BV]=Truncator(i.notes_id).chars(50);res[_BW]=Truncator(i.notes).chars(50);res['Payment Notes (id)']=Truncator(i.payment_notes_id).chars(50);res['Payment Notes (en)']=Truncator(i.payment_notes).chars(50);res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def agency_create(request):
	context={};context[_J]=_N;active_page=get_translated_active_page(_N);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def agency_update(request,uuid):
	context={};context[_J]=_N;active_page=get_translated_active_page(_N);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_Y;data=Agency.objects.filter(uuid=uuid);post=get_object_or_404(data)
	if request.method==_I:
...
def categories_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=Categories.objects.language(_B).create(name=request.POST.get(_H),status=request.POST.get(_S),site_id=site_id);post.set_current_language(_O);post.name=request.POST.get(_H);post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_j))
...
def product_ajax(request):
	site_id=get_site_id(request);obj=Product();obj.set_current_language(_O);subquery1=Subquery(ProductTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_H));subquery2=Subquery(ProductTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery3=Subquery(ProductTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery4=Subquery(ProductTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_U));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_A8).values(_i)[:1]);lang=obj.get_current_language();obj2=Product.objects.language(lang).filter(site_id=site_id).annotate(name_id=subquery1).annotate(title_id=subquery2).annotate(content_id=subquery3).annotate(sub_title_id=subquery4).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_AQ]=Truncator(i.name_id).chars(50);res[_AR]=Truncator(i.name).chars(50);res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_A0]=Truncator(i.sub_title).chars(50);res['price']=format_money(i.price,locale='ID');res[_p]=Truncator(i.content_id).chars(50);res[_q]=Truncator(i.content).chars(50);res[_A9]=i.is_header_text;res[_AW]=i.icon;res[_m]=i.file_path;res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
class CalendarView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;active_page=get_translated_active_page(_AC);context[_E]=active_page;context[_AS]=_P;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
...
def calendar_ajax(request):
	...
	for i in obj2:
		res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res['calendar Name']=Truncator(i.calendar_id).chars(50)
...
def calendar_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=GoogleCalendar.objects.update_or_create(calendar_id=request.POST.get(_Ap),defaults={_AB:request.FILES.get(_AB),'site_id':site_id});messages.info(request,mMsgBox.get(_f,request.POST.get(_Ap)));return redirect(reverse_lazy(_AC))
...
def calendar_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			if request.FILES.get(_AB):obj.file_path_doc=request.FILES.get(_AB)
			obj.site_id=site_id;obj.save();messages.info(request,mMsgBox.get(_Z,request.POST.get(_Ap)));return redirect(reverse_lazy(_AC))
...
def templateowner_ajax(request):
	...
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_AF]=Truncator(i.name).chars(50);res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def templateowner_create(request):
	context={};context[_J]=_Ac;active_page=get_translated_active_page(_B8);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def template_ajax(request):
	site_id=get_site_id(request);subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_r).values(_i)[:1]);obj2=Template.objects.filter(is_frontend=_P).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_AF]=Truncator(i.name).chars(50);res['Relative Path']=Truncator(i.rel_path).chars(50);res['Template Owner']=Truncator(i.template_owner).chars(50);res['Frontend']=Truncator(i.is_frontend).chars(50);res[_m]=i.file_path;res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def modellist_ajax(request):
	...
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_AF]=Truncator(i.name).chars(50);res[_BS]=Truncator(i.description).chars(50);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def modellist_create(request):
	context={};context[_J]=_Ad;active_page=get_translated_active_page(_Aq);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def is_have_group(request,*group_names):
	...
	if u:
		u=u.get()
		if bool(u.groups.filter(name__in=group_names))|u.is_superuser:return _P
	return _A
...
def service_ajax(request):
	...
	lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res['Domain']=i.site.domain;res['Service Kind']=i.get_kind_display();res['Agency']=Truncator(i.agency).chars(50);res['Expired Date']=get_natural_datetime(i.expired_date);res['Active']=i.is_active;res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def service_create(request):
	context={};context[_J]=_R;active_page=get_translated_active_page(_R);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
		form=ServiceForm(request.POST)
		if form.is_valid():post=Service.objects.create(site_id=request.POST.get(_AG),kind=request.POST.get('kind'),agency_id=request.POST.get(_N),is_active=bool(request.POST.get(_Ba)),expired_date=request.POST.get(_Bb));messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_R))
		else:pass
	else:messages.info(request,mMsgBox.get(_d));context[_G]=ServiceForm()
	return render(request,template,context)
def service_update(request,uuid):
	context={};context[_J]=_R;active_page=get_translated_active_page(_R);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_Y;data=Service.objects.filter(uuid=uuid);post=get_object_or_404(data)
	if request.method==_I:
...
def banner_ajax(request):
	site_id=get_site_id(request);subquery=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_u).values(_i));obj2=Banner.objects.filter(site_id=site_id).distinct().annotate(file_path=subquery);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res['Priority']=i.get_priority_display();res['Position']=i.get_position_display();res[_m]=i.file_path;res[_AK]=Truncator(i.link).chars(70);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def banner_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=form.save(commit=_A);post.site_id=site_id;post.admin_id=request.user.id;post.save();Photo.objects.create(content_object=post,file_path=request.POST.get(_M),description=request.POST.get(_T));messages.info(request,mMsgBox.get(_f,request.POST.get('position')));return redirect(reverse_lazy(_u))
...
class LocationView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
		context[_R]=service;context[_AS]=_P;active_page=get_translated_active_page(_s);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def location_ajax(request):
	site_id=get_site_id(request);lst=[];obj=Location();obj.set_current_language(_O);subquery1=Subquery(LocationTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(LocationTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_AL));lang=obj.get_current_language();obj2=Location.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(subtitle_id=subquery2)
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.subtitle_id).chars(50);res[_A0]=Truncator(i.subtitle).chars(50);tmp=Truncator(i.embed).chars(100);tmp=tmp.replace('<',' ');tmp=tmp.replace('>',' ');res['Embed']=tmp;res[_A9]=i.is_header_text;res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def whyus_ajax(request):
	site_id=get_site_id(request);obj=WhyUs();obj.set_current_language(_O);subquery=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_T));subquery3=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_U));lang=obj.get_current_language();obj2=WhyUs.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery).annotate(description_id=subquery2).annotate(sub_title_id=subquery3);lst=[]
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res[_n]=Truncator(i.title_id).chars(50);res[_o]=Truncator(i.title).chars(50);res[_z]=Truncator(i.sub_title).chars(50);res[_A0]=Truncator(i.sub_title_id).chars(50);res[_BV]=Truncator(i.description_id).chars(100);res[_BW]=Truncator(i.description).chars(100);res[_AW]=Truncator(i.icon).chars(20);res[_g]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def whyus_create(request):
	context={};context[_J]=_Ae;active_page=get_translated_active_page(_BB);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_c
	if request.method==_I:
...
def user_ajax(request):
	...
	for i in obj2:res={};res[_Q]=_C;res[_V]=i.uuid;res[_W]=i.updated_at;res['Email']=Truncator(i.email).chars(50);res[_AF]=Truncator(i.name).chars(50);res['Confirm']=i.email_confirmed;res['Site']=i.site.domain;res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def get_hitcount_monthly(request):
	B='created__month';A='created__year';lst=[];tgl=datetime.now();site_id=get_site_id(request);content_type_id=ContentType.objects.get(app_label=_BC,model=_AG);content_type_id=content_type_id.id if content_type_id else _C;hitcount_id=HitCount.objects.filter(content_type_id=content_type_id,object_pk=site_id).first();hitcount_id=hitcount_id.id if hitcount_id else _C;start_date=add_months(tgl,-3);start_date=datetime(start_date.year,start_date.month,1,0,0,0);res=calendar.monthrange(tgl.year,tgl.month);day=res[1];end_date=datetime(tgl.year,tgl.month,day,23,59,59);hit=Hit.objects.filter(hitcount_id=hitcount_id,created__range=[start_date,end_date]).values(A,B).annotate(count=Count(_B)).order_by(A,B);cat=[];val=[]
...
def template_lookup_ajax(request,service_id):
	search=request.GET.get(_B5);template=Template.objects.filter(service_option__contains=service_id,is_frontend=_P,status=OptStatusPublish.PUBLISHED)
	...
	return JsonResponse({_Ag:list(object_list),_B6:{'more':_P}},safe=_A)
def template_photo_ajax(request,template_id):subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_r).values(_i)[:1]);object_list=Template.objects.filter(id=template_id).values(_L).annotate(file_path=subquery_foto);return JsonResponse(list(object_list),safe=_A)
@transaction.atomic
def template_change(request,template_id):
	site_id=get_site_id(request);template=Template.objects.filter(is_frontend=_P,site__id=site_id)
	for i in template:i.site.remove(site_id)
	template=Template.objects.filter(id=template_id)
	if template:template=template.get();template.site.add(site_id);return JsonResponse('True',safe=_A)
	return JsonResponse('False',safe=_A)
...
def set_log(site_id,model_name,is_need_refresh):
	...
	model_list=model_list[0];tmp_log=Log.objects.filter(site_id=site_id,model_list=model_list);
