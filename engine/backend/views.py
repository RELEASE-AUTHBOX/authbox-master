...
def user_init_service(request,agency_id):
	...
	if request.method==_H:
		...
		if hostname.find('localhost')<0 and hostname.find('127.0.0.1')<0:
			...
			while not created:
				site,created=Site.objects.get_or_create(domain=tmp,defaults={_I:name})
			...
			else:site,created=Site.objects.get_or_create(domain=hostname,defaults={_I:hostname})
			print(_A9,site)
...
def tags_ajax(request):
	site_id=get_site_id(request);obj=Tags();obj.set_current_language(_N);subquery=Subquery(TagsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_I));lang=obj.get_current_language();obj2=Tags.objects.language(lang).filter(site_id=site_id).annotate(name_id=subquery);lst=[]
...
def tags_create(request):
	...
	if request.method==_H:
		...
		if form.is_valid():
			tmp=Tags.objects.filter(translations__name=request.POST.get(_I),site_id=site_id)
			if tmp:messages.info(request,mMsgBox.get(_BA));context[_G]=TagsForm()
			else:post=Tags.objects.language(_B).create(name=request.POST.get(_I),status=request.POST.get(_P),site_id=site_id);post.set_current_language(_N);post.name=request.POST.get(_I);post.save();messages.info(request,mMsgBox.get(_c,request.POST.get(_I)));return redirect(reverse_lazy(_h))
...
def tags_update(request,uuid):
	...
	if request.method==_H:
		...
		if form.is_valid():lang=request.POST.get(_f);obj=data.get();obj.set_current_language(lang);obj.name=request.POST.get(_I);obj.status=request.POST.get(_P);obj.site_id=site_id;obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_I)));return redirect(reverse_lazy(_h))
...
def logo_create(request):
	...
	if request.method==_H:
		...
		if form.is_valid():post=form.save(commit=_A);post.site_id=site_id;post.save();Photo.objects.create(content_object=post,file_path=request.POST.get(_M));messages.info(request,mMsgBox.get(_c,request.POST.get(_I)));return redirect(reverse_lazy(_s))
...
def logo_update(request,uuid):
	...
	if request.method==_H:
		...
		if form.is_valid():
			...
			messages.info(request,mMsgBox.get(_Y,request.POST.get(_I)));return redirect(reverse_lazy(_s))
	else:
...
def greeting_update(request,uuid):
	...
	if request.method==_H:
		...
		if form.is_valid():
			lang=request.POST.get(_f);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.content=request.POST.get(_K);obj.name=request.POST.get(_I);obj.designation=request.POST.get(_Al);obj.status=request.POST.get(_P);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save()
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M))
...
def relatedlink_ajax(request):
	site_id=get_site_id(request);lst=[];obj=RelatedLink();obj.set_current_language(_N);subquery1=Subquery(RelatedLinkTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_I));lang=obj.get_current_language();obj2=RelatedLink.objects.language(lang).filter(site_id=site_id).annotate(name_id=subquery1)
...
def document_update(request,uuid):
	...
	if request.method==_H:
		...
		if form.is_valid():
			lang=request.POST.get(_f);obj=data.get();obj.set_current_language(lang);obj.name=request.POST.get(_I);obj.content=request.POST.get(_K)
			if request.FILES.get(_Ab):obj.file_path_doc=request.FILES.get(_Ab)
			obj.status=request.POST.get(_P);obj.categories_id=request.POST.get(_g);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save();obj.size=os.stat(obj.file_path_doc.path).st_size;obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_I)));return redirect(reverse_lazy(_A7))
...
def menu_create(request):
	...
	if request.method==_H:
		...
		if form.is_valid():form_clean=form.cleaned_data;post=Menu.objects.language(_B).create(name=form_clean[_I],parent_id=request.POST.get(_AX),link=form_clean[_z],order_menu=form_clean[_As],icon=form_clean[_R],is_visibled=form_clean[_At],is_external=form_clean[_BH],exclude_menu=form_clean[_Au]);post.menu_group.add(menu_group);post.set_current_language(_N);post.name=form_clean[_I];post.save();messages.info(request,mMsgBox.get(_c,request.POST.get(_I)));return redirect(reverse_lazy(_D))
...
def categories_ajax(request):
	site_id=get_site_id(request);obj=Categories();obj.set_current_language(_N);subquery1=Subquery(CategoriesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_I));lang=obj.get_current_language();obj2=Categories.objects.language(lang).filter(site_id=site_id).annotate(name_id=subquery1);lst=[]
	for i in obj2:res={};res[_R]=_C;res[_S]=i.uuid;res[_T]=i.updated_at;res[_AH]=Truncator(i.name_id).chars(50);res[_AI]=Truncator(i.name).chars(50);res[_e]=i.get_status_display();res[_b]=get_natural_datetime(i.updated_at);res[_U]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def categories_create(request):
	...
	if request.method==_H:
		...
		if form.is_valid():post=Categories.objects.language(_B).create(name=request.POST.get(_I),status=request.POST.get(_P),site_id=site_id);post.set_current_language(_N);post.name=request.POST.get(_I);post.save();messages.info(request,mMsgBox.get(_c,request.POST.get(_I)));return redirect(reverse_lazy(_g))
...
def categories_update(request,uuid):
	...
	if request.method==_H:
		...
		if form.is_valid():lang=request.POST.get(_f);obj=data.get();obj.set_current_language(lang);obj.name=request.POST.get(_I);obj.status=request.POST.get(_P);obj.site_id=site_id;obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_I)));return redirect(reverse_lazy(_g))
...
def templateowner_create(request):
	...
	if request.method==_H:
		...
		if form.is_valid():post=TemplateOwner.objects.create(name=request.POST.get(_I));messages.info(request,mMsgBox.get(_c,request.POST.get(_I)));return redirect(reverse_lazy(_AS))
...
def templateowner_update(request,uuid):
	...
	if request.method==_H:
		...
		if form.is_valid():obj=data.get();obj.name=request.POST.get(_I);obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_I)));return redirect(reverse_lazy(_AS))
...
def template_update(request,uuid):
	...
	if request.method==_H:
		...
		if form.is_valid():
			obj=data.get();tmp=request.POST.get(_Ax);obj.name=request.POST.get(_I);obj.rel_path=request.POST.get('rel_path');obj.is_frontend=bool(request.POST.get(_BL))
			if tmp:obj.template_owner_id=request.POST.get(_Ax)
			obj.save()
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M))
			messages.info(request,mMsgBox.get(_Y,request.POST.get(_I)));return redirect(reverse_lazy(_q))
	else:
		messages.info(request,mMsgBox.get(_Z))
		if post_photo:context[_L]=PhotoForm(instance=post_photo)
		else:context[_L]=PhotoForm()
...
def modellist_create(request):
	...
	if request.method==_H:
		...
		if form.is_valid():post=ModelList.objects.create(name=request.POST.get(_I),description=request.POST.get(_AE),status=request.POST.get(_P));messages.info(request,mMsgBox.get(_c,request.POST.get(_I)));return redirect(reverse_lazy(_AT))
...
def modellist_update(request,uuid):
	...
	if request.method==_H:
		...
		if form.is_valid():obj=data.get();obj.name=request.POST.get(_I);obj.description=request.POST.get(_AE);obj.status=request.POST.get(_P);obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_I)));return redirect(reverse_lazy(_AT))
...
def user_update(request,uuid):
	...
	if request.method==_H:
		...
		if form.is_valid():obj=data.get();obj.name=request.POST.get(_I);obj.email=request.POST.get(_Ac);obj.save();messages.info(request,mMsgBox.get(_Y,request.POST.get(_I)));return redirect(reverse_lazy(_r))
