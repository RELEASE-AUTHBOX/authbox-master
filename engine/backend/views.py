...
def get_template_info(site_id):
	subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_q).values(_h)[:1]);template=Template.objects.filter(site__id=site_id,is_frontend=_T).annotate(file_path=subquery_foto)[:1];
	if template:return template[0]
def get_init_template_id(site_id):
	A='template=';template=Template.objects.filter(site__id=site_id,status=OptStatusPublish.PUBLISHED,is_frontend=_T)[:1];
	if template:template=template.get();return{_A:template.id,_AD:template.name}
class IndexView(TemplateView):
	site_id=_C
	def get(self,request,*args,**kwargs):
		self.site_id=get_site_id(request);
		if self.site_id==-1:return redirect(reverse_lazy(_AC))
		elif self.site_id==-2:return redirect(_Ar)
		elif self.site_id==-3 or self.site_id==-31:return redirect(reverse_lazy('user_init_agency'))
		elif self.site_id==-4 or self.site_id==-41:return redirect(reverse_lazy('user_init_service',kwargs={_BK:get_agency_from(request)}))
...
def user_init_agency(request):
	...
	if request.method==_I:
		...
		if created:user.agency.add(agency);return redirect(reverse_lazy(_AC))
...
def user_init_service_ajax(request,agency_id,service_id):
	mfound=_B;user=User.objects.get(id=request.user.id);agency=user.agency.filter(is_default=_T)[:1]
	if agency:
		agency=agency.get()
		if agency.id==agency_id:mfound=_T
	if mfound:
		service=Service.objects.filter(agency_id=agency.id,is_default=_T)
...
def service_change_ajax(request,service_id):
	lst=[];temp=Template.objects.filter(is_frontend=_T)
	for i in temp:
		if service_id in i.service_option:res={};res[_A]=i.id;res[_AD]=i.name;lst.append(res)
	return JsonResponse({_Af:lst},safe=_B)
...
def tags_create(request):
	context={};context[_J]=_k;active_page=get_translated_active_page(_k);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
		...
		if form.is_valid():
			...
			else:post=Tags.objects.language(_A).create(name=request.POST.get(_H),status=request.POST.get(_R),site_id=site_id);post.set_current_language(_O);post.name=request.POST.get(_H);post.save();messages.info(request,mMsgBox.get(_d,request.POST.get(_H)));return redirect(reverse_lazy(_k))
...
def logo_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_S]=i.uuid;res[_U]=i.updated_at;res[_AE]=Truncator(i.name).chars(50);res[_l]=i.file_path;res[_Z]=get_natural_datetime(i.updated_at);res[_V]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
...
def fasilities_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_g);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_e);obj.content=request.POST.get(_K);obj.is_header_text=form.cleaned_data[_j];obj.status=request.POST.get(_R);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save()
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M))
...
def news_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_S]=i.uuid;res[_U]=i.updated_at;res[_m]=Truncator(i.title_id).chars(50);res[_n]=Truncator(i.title).chars(50);res[_o]=Truncator(i.content_id).chars(50);res[_p]=Truncator(i.content).chars(50);res[_l]=i.file_path;res['Header']=i.is_header_text;res[_f]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_V]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
...
def slideshow_ajax(request):
	site_id=get_site_id(request);lst=[];obj=SlideShow();obj.set_current_language(_O);subquery1=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery3=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_e));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model='slideshow').values(_h)[:1]);lang=obj.get_current_language();obj2=SlideShow.objects.language(lang).filter(site_id=site_id).distinct().annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto)
...
def slideshow_create(request):
	context={};context[_J]=_AS;active_page=get_translated_active_page(_Av);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
...
def slideshow_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_g);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_e);obj.content=request.POST.get(_K);obj.status=request.POST.get(_R);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save()
...
def dailyalert_create(request):
	context={};context[_J]=_AT;active_page=get_translated_active_page(_Aw);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
...
def pages_ajax(request):
	site_id=get_site_id(request);obj=Pages();obj.set_current_language(_O);subquery1=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery3=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_e));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_A5).values(_h)[:1]);lang=obj.get_current_language();obj2=Pages.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_S]=i.uuid;res[_U]=i.updated_at;res[_m]=Truncator(i.title_id).chars(50);res[_n]=Truncator(i.title).chars(50);res[_y]=Truncator(i.sub_title_id).chars(50);res[_z]=Truncator(i.sub_title).chars(50);res[_o]=Truncator(i.content_id).chars(50);res[_p]=Truncator(i.content).chars(50);res[_l]=i.file_path;res[_f]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_V]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
...
def pages_create(request):
	context={};context[_J]=_A5;active_page=get_translated_active_page(_A5);context[_E]=active_page;context[_Ag]=_BN;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
...
def socialmedia_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=form.save(commit=_B);post.site_id=site_id;post.save();messages.info(request,mMsgBox.get(_d,request.POST.get(_s)));return redirect(reverse_lazy(_AU))
...
def howitworks_ajax(request):
	site_id=get_site_id(request);obj=HowItWorks();subquery=Subquery(HowItWorksTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(HowItWorksTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery3=Subquery(HowItWorksTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_e));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model='howitworks').values(_h)[:1]);lang=obj.get_current_language();obj2=HowItWorks.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_S]=i.uuid;res[_U]=i.updated_at;res[_m]=Truncator(i.title_id).chars(50);res[_n]=Truncator(i.title).chars(50);res[_y]=Truncator(i.sub_title_id).chars(50);res[_z]=Truncator(i.sub_title).chars(50);res[_o]=Truncator(i.content_id).chars(70);res[_p]=Truncator(i.content).chars(70);res[_AV]=i.icon;res[_A8]=i.is_header_text;res[_Au]=i.order_item;res[_l]=i.file_path;res[_f]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_V]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def howitworks_create(request):
	context={};context[_J]=_AW;active_page=get_translated_active_page(_Az);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
...
def howitworks_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_g);obj=data.get();obj.set_current_language(lang);obj.icon=request.POST.get(_P);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_e);obj.content=request.POST.get(_K);obj.is_header_text=form.cleaned_data[_j];order_item=request.POST.get(_A_)
...
def aboutus_ajax(request):
	site_id=get_site_id(request);obj=AboutUs();subquery=Subquery(AboutUsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(AboutUsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_e));subquery3=Subquery(AboutUsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model='aboutus').values(_h)[:1]);lang=obj.get_current_language();obj2=AboutUs.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery).annotate(sub_title_id=subquery2).annotate(content_id=subquery3).annotate(file_path=subquery_foto);lst=[]
...
def aboutus_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_g);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_e);obj.content=request.POST.get(_K);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save()
			if photo.is_valid():
				if request.POST.get(_M):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_M))
...
def photogallery_create(request):
	context={};context[_J]=_AY;active_page=get_translated_active_page(_B1);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
...
def videogallery_create(request):
	context={};context[_J]=_AZ;active_page=get_translated_active_page(_B2);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
...
def relatedlink_create(request):
	context={};context[_J]=_Aa;active_page=get_translated_active_page(_B3);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
...
def application_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_S]=i.uuid;res[_U]=_C;res[_AE]=i.get_name_display();res['Value']=i.value;res[_BR]=i.ref_template_block.description;res[_l]=i.file_path;res[_Z]=get_natural_datetime(i.updated_at);res[_V]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def application_update(request,uuid):
	C='template_block_model_list_id';B='template_id';A='select2_global_setting';context={};context[_J]=_AK;active_page=get_translated_active_page(_AK);context[_E]=active_page;site_id=get_site_id(request);context[A]='Template Block';template_id=get_template_id(site_id);context[B]=template_id;menu=get_menu_caches(request,_AK,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_W;data=GlobalSetting.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);template_block=TemplateBlock.objects.filter(id=post.ref_template_block.id);template_block_model_list_id='0'
	if template_block:
...
def menu_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():form_clean=form.cleaned_data;post=Menu.objects.language(_A).create(name=form_clean[_H],parent_id=request.POST.get(_Ah),link=form_clean[_s],order_menu=form_clean[_Al],icon=form_clean[_P],is_visibled=form_clean[_AM],is_external=form_clean[_BT],exclude_menu=form_clean[_B6]);post.menu_group.add(menu_group);post.set_current_language(_O);post.name=form_clean[_H];post.save();messages.info(request,mMsgBox.get(_d,request.POST.get(_H)));return redirect(reverse_lazy(_D))
...
def agency_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_S]=i.uuid;res[_U]=i.updated_at;res[_AE]=Truncator(i.name).chars(50);res['Address (id)']=Truncator(i.address_id).chars(50);res['Address (en)']=Truncator(i.address).chars(50);res[_BU]=Truncator(i.notes_id).chars(50);res[_BV]=Truncator(i.notes).chars(50);res['Payment Notes (id)']=Truncator(i.payment_notes_id).chars(50);res['Payment Notes (en)']=Truncator(i.payment_notes).chars(50);res[_Z]=get_natural_datetime(i.updated_at);res[_V]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
...
def categories_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=Categories.objects.language(_A).create(name=request.POST.get(_H),status=request.POST.get(_R),site_id=site_id);post.set_current_language(_O);post.name=request.POST.get(_H);post.save();messages.info(request,mMsgBox.get(_d,request.POST.get(_H)));return redirect(reverse_lazy(_i))
...
def product_ajax(request):
	site_id=get_site_id(request);obj=Product();obj.set_current_language(_O);subquery1=Subquery(ProductTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_H));subquery2=Subquery(ProductTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery3=Subquery(ProductTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_K));subquery4=Subquery(ProductTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_e));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_A),content_type__model=_A7).values(_h)[:1]);lang=obj.get_current_language();obj2=Product.objects.language(lang).filter(site_id=site_id).annotate(name_id=subquery1).annotate(title_id=subquery2).annotate(content_id=subquery3).annotate(sub_title_id=subquery4).annotate(file_path=subquery_foto);lst=[]
...
def calendar_create(request):
	context={};context[_J]=_AB;active_page=get_translated_active_page(_AB);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
		form=GoogleCalendarForm(request.POST,request.FILES)
		if form.is_valid():post=GoogleCalendar.objects.update_or_create(calendar_id=request.POST.get(_Ao),defaults={_AA:request.FILES.get(_AA),'site_id':site_id});messages.info(request,mMsgBox.get(_d,request.POST.get(_Ao)));return redirect(reverse_lazy(_AB))
...
def calendar_update(request,uuid):
	...
	return render(request,template,context)
def calendar_delete(request,uuid):context={};site_id=get_site_id(request);data=GoogleCalendar.objects.filter(uuid=uuid);post=get_object_or_404(data);tmp=post.calendar_id;post.delete();messages.info(request,mMsgBox.get(_c,tmp));return redirect(reverse_lazy(_AB))
...
def templateowner_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_S]=i.uuid;res[_U]=i.updated_at;res[_AE]=Truncator(i.name).chars(50);res[_Z]=get_natural_datetime(i.updated_at);res[_V]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def templateowner_create(request):
	context={};context[_J]=_Ab;active_page=get_translated_active_page(_B7);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
...
def modellist_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_S]=i.uuid;res[_U]=i.updated_at;res[_AE]=Truncator(i.name).chars(50);res[_BR]=Truncator(i.description).chars(50);res[_f]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_V]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def modellist_create(request):
	context={};context[_J]=_Ac;active_page=get_translated_active_page(_BA);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
...
def service_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=Service.objects.create(site_id=request.POST.get(_AF),kind=request.POST.get('kind'),agency_id=request.POST.get(_N),is_active=bool(request.POST.get(_BY)),expired_date=request.POST.get(_BZ));messages.info(request,mMsgBox.get(_d,request.POST.get(_H)));return redirect(reverse_lazy(_Q))
		else:pass
	else:messages.info(request,mMsgBox.get(_b));context[_G]=ServiceForm()
	return render(request,template,context)
...
def whyus_ajax(request):
	site_id=get_site_id(request);obj=WhyUs();obj.set_current_language(_O);subquery=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_F));subquery2=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_AL));subquery3=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_A),language_code=_A).values(_e));lang=obj.get_current_language();obj2=WhyUs.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery).annotate(description_id=subquery2).annotate(sub_title_id=subquery3);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_S]=i.uuid;res[_U]=i.updated_at;res[_m]=Truncator(i.title_id).chars(50);res[_n]=Truncator(i.title).chars(50);res[_y]=Truncator(i.sub_title).chars(50);res[_z]=Truncator(i.sub_title_id).chars(50);res[_BU]=Truncator(i.description_id).chars(100);res[_BV]=Truncator(i.description).chars(100);res[_AV]=Truncator(i.icon).chars(20);res[_f]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_V]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
def whyus_create(request):
	context={};context[_J]=_Ad;active_page=get_translated_active_page(_BB);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
...
def user_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_S]=i.uuid;res[_U]=i.updated_at;res['Email']=Truncator(i.email).chars(50);res[_AE]=Truncator(i.name).chars(50);res['Confirm']=i.email_confirmed;res['Site']=i.site.domain;res[_Z]=get_natural_datetime(i.updated_at);res[_V]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
