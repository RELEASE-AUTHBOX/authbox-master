...
def user_init_agency_ajax(request,agency_id):
	...
	return JsonResponse({_Av:_N},safe=_A)
@transaction.atomic
...
def application_update_ajax(request):
	site_id=get_site_id(request);template=get_template(site_id,is_frontend=_N);search=request.GET.get(_B6);template_id=request.GET.get('t');modellist_id=request.GET.get('ml')
	...
	return JsonResponse({_Ah:list(object_list),_B7:{'more':_N}},safe=_A)
...
def menu_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():form_clean=form.cleaned_data;post=Menu.objects.language(_B).create(name=form_clean[_H],parent_id=request.POST.get(_Aj),link=form_clean[_t],order_menu=form_clean[_An],icon=form_clean[_R],is_visibled=form_clean[_AO],is_external=form_clean[_BV],exclude_menu=form_clean[_B8]);post.menu_group.add(menu_group);post.set_current_language(_P);post.name=form_clean[_H];post.save();messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_D))
...
def template_lookup_ajax(request,service_id):
	search=request.GET.get(_B6);template=Template.objects.filter(service_option__contains=service_id,is_frontend=_N,status=OptStatusPublish.PUBLISHED)
