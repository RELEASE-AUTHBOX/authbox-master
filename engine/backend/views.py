...
def howitworks_create(request):
	context={};context[_J]=_AW;active_page=get_translated_active_page(_Az);print(_E,active_page);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_B)+_a
	if request.method==_I:
...
def application_ajax(request):
	...
	for i in obj2:print('ref',i.ref_template_block.id);print('template_block',i.file_path);res={};res[_P]=_C;res[_S]=i.uuid;res[_U]=_C;res[_AE]=i.get_name_display();res['Value']=i.value;res[_BR]=i.ref_template_block.description;res[_l]=i.file_path;res[_Z]=get_natural_datetime(i.updated_at);res[_V]=_C;lst.append(res)
	return JsonResponse(lst,safe=_B)
