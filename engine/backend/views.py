...
class IndexView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super(IndexView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_A9);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);agency=get_agency(self.request);context[_k]=agency;service=[]
...
class TagsView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super(TagsView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_e);context[_E]=active_page;user=User.objects.get(id=self.request.user.id);agency=user.agency.all();context[_k]=agency;service=[]
		for i in agency:service.append(Service.objects.filter(agency_id=i.id))
...
def greeting_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_c);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.content=request.POST.get(_K);obj.name=request.POST.get(_H);obj.designation=request.POST.get(_Af);obj.status=request.POST.get(_O);obj.site_id=site_id;obj.admin_id=request.user.id;obj.save()
			if photo.is_valid():
				if request.POST.get(_N):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_N))
...
def agency_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_A5]=Truncator(i.name).chars(50);res['Address (id)']=Truncator(i.address_id).chars(50);res['Address (en)']=Truncator(i.address).chars(50);res[_BE]=Truncator(i.notes_id).chars(50);res[_BF]=Truncator(i.notes).chars(50);res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def template_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():tmp=request.POST.get(_Ar);post=Template.objects.create(name=request.POST.get(_H),rel_path=request.POST.get('rel_path'),is_frontend=bool(request.POST.get(_BH)),template_owner_id=tmp if tmp else _C);messages.info(request,mMsgBox.get(_a,request.POST.get(_H)));return redirect(reverse_lazy(_AP))
...
def template_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			obj=data.get();tmp=request.POST.get(_Ar);obj.name=request.POST.get(_H);obj.rel_path=request.POST.get('rel_path');obj.is_frontend=bool(request.POST.get(_BH))
...
def whyus_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_g]=Truncator(i.title_id).chars(50);res[_h]=Truncator(i.title).chars(50);res[_BE]=Truncator(i.description_id).chars(100);res[_BF]=Truncator(i.description).chars(100);res[_AX]=Truncator(i.icon).chars(20);res[_b]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
