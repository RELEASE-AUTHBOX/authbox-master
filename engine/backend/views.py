...
def get_template_info(site_id):
	subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_r).values(_g)[:1]);template=Template.objects.filter(site__id=site_id,is_frontend=_O).annotate(file_path=subquery_foto)[:1];
	if template:return template[0]
	return _C
def get_init_template_id(site_id):
	A='template=';template=Template.objects.filter(site__id=site_id,status=OptStatusPublish.PUBLISHED,is_frontend=_O)[:1];
...
class IndexView(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		...
		elif self.site_id==-4 or self.site_id==-41:return redirect(reverse_lazy('user_init_service',kwargs={_BM:get_agency_from(request)}))
...
def user_init_service_ajax(request,agency_id,service_id):
	mfound=_A;user=User.objects.get(id=request.user.id);agency=user.agency.filter(is_default=_O)[:1]
	if agency:
		agency=agency.get()
		if agency.id==agency_id:mfound=_O
	if mfound:
		service=Service.objects.filter(agency_id=agency.id,is_default=_O)
...
def service_change_ajax(request,service_id):
	lst=[];temp=Template.objects.filter(is_frontend=_O)
	for i in temp:
		if service_id in i.service_option:res={};res[_B]=i.id;res[_AE]=i.name;lst.append(res)
	return JsonResponse({_Ah:lst},safe=_A)
...
def application_ajax(request):
	...
	for i in obj2:res={};res[_R]=_C;res[_V]=i.uuid;res[_W]=_C;res[_AF]=i.get_name_display();res['Value']=i.value;res[_BT]=i.ref_template_block.description;res[_m]=i.file_path;res[_b]=get_natural_datetime(i.updated_at);res[_X]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def application_update(request,uuid):
	C='template_block_model_list_id';B='template_id';A='select2_global_setting';context={};context[_J]=_AN;active_page=get_translated_active_page(_AN);context[_E]=active_page;site_id=get_site_id(request);context[A]='Template Block';template_id=get_template_id(site_id);context[B]=template_id;menu=get_menu_caches(request,_AN,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_Y;data=GlobalSetting.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);template_block=TemplateBlock.objects.filter(id=post.ref_template_block.id);template_block_model_list_id='0'
	if template_block:
...
def service_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=Service.objects.create(site_id=request.POST.get(_AG),kind=request.POST.get('kind'),agency_id=request.POST.get(_P),is_active=bool(request.POST.get(_Bb)),expired_date=request.POST.get(_Bc));messages.info(request,mMsgBox.get(_f,request.POST.get(_H)));return redirect(reverse_lazy(_S))
		else:pass
