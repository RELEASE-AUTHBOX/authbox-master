...
class WhyUsForm(TranslatableModelForm):
	class Meta:model=WhyUs;fields=[_F,_I,_N,_Q,_B]
...
class AnnouncementForm(TranslatableModelForm):
	...
	class Meta:model=Announcement;fields=[_F,_A,_H,_J,_R,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
	def __init__(self,*args,**kwargs):
		site_id=kwargs.pop(_O);super().__init__(*(args),**kwargs)
		if site_id:self.fields[_J].queryset=Tags.objects.filter(site_id=site_id);self.fields[_H].queryset=Categories.objects.filter(site_id=site_id)
class FasilitiesForm(TranslatableModelForm):
	dim_w=forms.CharField(widget=forms.HiddenInput(),initial=870);dim_h=forms.CharField(widget=forms.HiddenInput(),initial=500)
	class Meta:model=Fasilities;fields=[_F,_I,_A,_L,_K,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
class OffersForm(TranslatableModelForm):
	dim_w=forms.CharField(widget=forms.HiddenInput(),initial=870);dim_h=forms.CharField(widget=forms.HiddenInput(),initial=500)
	class Meta:model=Offers;fields=[_F,_I,_A,_L,_K,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
class NewsForm(TranslatableModelForm):
	dim_w=forms.CharField(widget=forms.HiddenInput(),initial=870);dim_h=forms.CharField(widget=forms.HiddenInput(),initial=500)
	class Meta:model=News;fields=[_F,_A,_H,_J,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
	def __init__(self,*args,**kwargs):
		site_id=kwargs.pop(_O);super().__init__(*(args),**kwargs)
		if site_id:self.fields[_J].queryset=Tags.objects.filter(site_id=site_id);self.fields[_H].queryset=Categories.objects.filter(site_id=site_id)
class TestimonyForm(TranslatableModelForm):
	...
	class Meta:model=Testimony;fields=[_F,_S,_A,_K,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
class ArticleForm(TranslatableModelForm):
	dim_w=forms.CharField(widget=forms.HiddenInput(),initial=870);dim_h=forms.CharField(widget=forms.HiddenInput(),initial=500)
	class Meta:model=Article;fields=[_F,_A,_H,_J,_K,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
	def __init__(self,*args,**kwargs):
		site_id=kwargs.pop(_O);super().__init__(*(args),**kwargs)
		if site_id:self.fields[_J].queryset=Tags.objects.filter(site_id=site_id);self.fields[_H].queryset=Categories.objects.filter(site_id=site_id)
class EventsForm(TranslatableModelForm):
	dim_w=forms.CharField(widget=forms.HiddenInput(),initial=870);dim_h=forms.CharField(widget=forms.HiddenInput(),initial=500)
	class Meta:model=Events;fields=[_F,'location','date','time',_A,_H,_J,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
	def __init__(self,*args,**kwargs):
		site_id=kwargs.pop(_O);super().__init__(*(args),**kwargs)
		if site_id:self.fields[_J].queryset=Tags.objects.filter(site_id=site_id);self.fields[_H].queryset=Categories.objects.filter(site_id=site_id)
class SlideShowForm(TranslatableModelForm):
	...
	class Meta:model=SlideShow;fields=[_F,_I,_A,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
...
class PagesForm(TranslatableModelForm):
	dim_w=forms.CharField(widget=forms.HiddenInput(),initial=870);dim_h=forms.CharField(widget=forms.HiddenInput(),initial=500)
	class Meta:model=Pages;fields=[_F,_I,_A,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
...
class HowItWorksForm(TranslatableModelForm):
	...
	class Meta:model=HowItWorks;fields=[_F,_I,_A,_N,_L,_K,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
class AboutUsForm(TranslatableModelForm):
	...
	class Meta:model=AboutUs;fields=[_F,_I,_A,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
class PhotoGalleryForm(TranslatableModelForm):
	...
	class Meta:model=PhotoGallery;fields=[_F,_A,_L,_K,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
...
class ProductForm(TranslatableModelForm):
	dim_w=forms.CharField(widget=forms.HiddenInput(),initial=870);dim_h=forms.CharField(widget=forms.HiddenInput(),initial=500)
	class Meta:model=Product;fields=[_G,_F,_I,_N,'price',_A,_L,_K,_B];widgets={_A:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
...
class LocationForm(TranslatableModelForm):
	class Meta:model=Location;fields=[_F,_S,_P,_K,_B];widgets={_P:CKEditor5Widget(attrs={_C:_D},config_name=_E)}
