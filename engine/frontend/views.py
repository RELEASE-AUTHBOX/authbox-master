...
def get_calendar_ajax(request,year,month):
	...
	return JsonResponse(res,safe=_F)
...
def get_logo(site_id,max_data=1):
	subquery_foto=get_photo(_N);logo=Logo.objects.filter(site_id=site_id).values(_f).annotate(file_path=subquery_foto)[:max_data]
	if logo:return logo
	return _A
def get_logo_pos(site_id,options):
	subquery_foto=get_photo(_N);logo=Logo.objects.filter(site_id=site_id).values(_f).annotate(file_path=subquery_foto).filter(pos=options)
...
def get_statistic(site_id,is_cache=_F):
...
def get_announcement(site_id,lang,max_data=3,max_words=20):
	...
	return obj
def get_slideshow(site_id,lang,max_data=5,is_random=_F):
...
def get_fasilities(site_id,lang,exclude_id=[],is_header_text=_A,is_shuffle=_A):
	print('exclude_id',exclude_id);subquery_foto=get_photo(_Y)
...
def get_testimony(site_id,lang,max_data=3,max_words=20):
	...
	for i in obj:
		if not i.is_header_text:i.content=Truncator(strip_tags(i.content)).words(max_words);i.created_at=get_natural_datetime(i.created_at)
	return obj
def get_product(site_id,lang,max_data=3,max_words=20):subquery_foto=get_photo(_a);obj=Product.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_B,_R)[:max_data];return obj
def get_greeting(site_id,lang,max_data=1):subquery_foto=get_photo(_b);return Greeting.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_D)[:max_data]
...
def get_article(site_id,lang,max_data=3,max_words=20):
	subquery_foto=get_photo(_K);obj=Article.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_B,_D)[:max_data]
	for i in obj:
		if not i.is_header_text:i.content=Truncator(strip_tags(i.content)).words(max_words);i.created_at=get_natural_datetime(i.created_at)
	return obj
...
def get_related_model(site_id,lang,max_data,model,kind,exclude_slug=_A):
	subquery_foto=get_photo(kind)
	if exclude_slug:qs=model.objects.translated(lang).filter(site_id=site_id,is_header_text=_F).exclude(slug=exclude_slug)
	else:qs=model.objects.translated(lang).filter(site_id=site_id,is_header_text=_F)
...
def get_content_list(site_id,lang,model,kind,slug):
	...
	field_is_header_text_exists=_F
	for field in model._meta.get_fields():
		if field.name=='is_header_text':field_is_header_text_exists=_H;break
	subquery_foto=get_photo(kind)
	if slug==_r:
		if field_is_header_text_exists:return model.objects.translated(lang).filter(site_id=site_id,is_header_text=_F).annotate(file_path=subquery_foto).order_by(_T)
		return model.objects.translated(lang).filter(site_id=site_id).annotate(file_path=subquery_foto).order_by(_T)
	else:
		categories=Categories.objects.filter(slug=slug);categories=categories.get()if categories else _A
		if categories:
			if field_is_header_text_exists:return model.objects.translated(lang).filter(site_id=site_id,categories_id=categories.id,is_header_text=_F).annotate(file_path=subquery_foto).order_by(_T)
			return model.objects.translated(lang).filter(site_id=site_id,categories_id=categories.id).annotate(file_path=subquery_foto).order_by(_T)
...
class IndexView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super().get_context_data(*(args),**kwargs);context[_g]='index';context['menu_justify']='right';context['agency_meta']=get_agency_meta(self.request,self.site_id);context[_L]=self.request.device[_L];print('OptSettingName',OptSettingName)
		for i in OptSettingName:
		...
		active_page=get_translated_active_page(_U);menu=get_menu_caches(self.request,_V,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);parent_order=1;menu_footer0=get_menu_caches_footer2(self.request,f"header_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer0);parent_order=2;menu_footer1=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer1);parent_order=3;menu_footer2=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer2);agency=get_agency_info(self.site_id);context.update(agency);context[_N]=get_logo(self.site_id);context['logo_top_normal']=get_logo_pos(self.site_id,OptLogoSettingPos.TOP_NORMAL);context['logo_top_dark']=get_logo_pos(self.site_id,OptLogoSettingPos.TOP_DARK);context['logo_bottom_normal']=get_logo_pos(self.site_id,OptLogoSettingPos.BOTTOM_NORMAL);context['logo_bottom_dark']=get_logo_pos(self.site_id,OptLogoSettingPos.BOTTOM_DARK);context['format_date']=get_format_date();lang=get_active_language_choices()[0];context[_z]=get_banner(self.site_id);context[_C]=get_announcement(self.site_id,lang,max_data=10);context[_A2]=get_autoheadline(self.site_id,lang);context['article_notes']=get_article_notes(self.site_id,lang);context[_O]=get_slideshow(self.site_id,lang,max_data=10);context['dailyalert']=get_dailyalert(self.site_id,lang,max_data=10);context[_h]=get_howitworks(self.site_id,lang,max_data=10);context[_l]=get_aboutus(self.site_id,lang,max_data=5);context[_m]=get_testimony(self.site_id,lang,max_data=10);context[_a]=get_product(self.site_id,lang,max_data=10);context['whyus']=get_whyus(self.site_id,lang,max_data=10);context[_Y]=get_fasilities(self.site_id,lang);context[_Z]=get_offers(self.site_id,lang);context[_b]=get_greeting(self.site_id,lang);context[_P]=get_events(self.site_id,lang);context[_J]=get_photogallery(self.site_id,lang);print('!!! photo gallery !!! ====',context[_J]);context[_A0]=get_videogallery(self.site_id,lang);context[_W]=get_relatedlink(self.site_id,lang);context[_E]=get_news(self.site_id,lang,max_data=10);context[_K]=get_article(self.site_id,lang,max_data=10);context[_i]=get_document(self.site_id,lang,max_data=10);context[_X]=get_socialmedia(self.site_id,max_data=10);context[_o]=get_location(self.site_id,lang);context[_M]=get_base_url(self.request);print(_t,context[_M]);og=get_og(self.site_id);context.update(og);return context
class CheckOutView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		B='footer_menu_2';A='footer_menu_1';context=super().get_context_data(*(args),**kwargs);context[_g]='checkout';context[_L]=self.request.device[_L];active_page=get_translated_active_page(_U);menu=get_menu_caches(self.request,_V,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);menu_footer1=get_menu_caches_footer(self.request,A,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=A);context.update(menu_footer1);menu_footer2=get_menu_caches_footer(self.request,B,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=B);context.update(menu_footer2);context[_d]=get_menu_group(self.site_id);slug=self.kwargs[_S]
		if not slug:raise Http404(_u%(self.request.get_host(),_k))
		kind=self.kwargs[_I];context[_I]=kind;model=apps.get_model(_G,kind);agency=get_agency_info(self.site_id);context.update(agency);context[_N]=get_logo(self.site_id);lang=get_active_language_choices()[0];content_detail=get_content_detail(self.site_id,lang,model,kind,slug);context[_v]=content_detail;context[_W]=get_relatedlink(self.site_id,lang);context[_X]=get_socialmedia(self.site_id);context[_O]=get_slideshow(self.site_id,lang);context[_M]=get_base_url(self.request,1);print(_t,context[_M]);return context
class DetailView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		B='random_paint_kind';A='latest_kind_kind';context=super(DetailView,self).get_context_data(*(args),**kwargs);context[_g]='detail';context[_L]=self.request.device[_L];active_page=get_translated_active_page(_U);menu=get_menu_caches(self.request,_V,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);parent_order=1;menu_footer1=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer1);parent_order=2;menu_footer2=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer2);context[_d]=get_menu_group(self.site_id);slug=self.kwargs[_S]
		if not slug:raise Http404(_u%(self.request.get_host(),_k))
		agency=get_agency_info(self.site_id);context.update(agency);context[_N]=get_logo(self.site_id);lang=get_active_language_choices()[0];model_with_categories=[_C,_E,_K,_P,_i,_j];model_with_content=[_C,_E,_K,_P,_O,_b,_j,_J,_Y,_Z,_h,_l,_m,_a,_i];model_randomize=[_E,_K,_P,_Y,_Z,_h,_a,_j];kind=self.kwargs[_I];print('kind detail',kind)
		if kind in model_with_content:context[_I]=kind
		context[_C]=get_announcement(self.site_id,lang,6);model=apps.get_model(_G,kind);latest_kind=_A;random_paint=_A;print('1',model_randomize)
		...
		if not latest_kind:
			...
			for i in model_randomize:
				print(_e,i);tmp_kind=i;tmp_model=apps.get_model(_G,i);latest_kind=get_latest_model(self.site_id,lang,10,tmp_model,i)
		...
		if not random_paint:
			random.shuffle(model_randomize)
			for i in model_randomize:
				print(_e,i);tmp_kind=i;tmp_model=apps.get_model(_G,i);random_paint=get_related_model(self.site_id,lang,4,tmp_model,i)
		...
		for i in model_randomize:
			print(_e,i);tmp_model=apps.get_model(_G,i);random_model=get_model_content(self.site_id,lang,tmp_model,i,10)
			if random_model:break
		context[_A7]=random_model;content_detail=get_content_detail(self.site_id,lang,model,kind,slug);context[_v]=content_detail;context[_J]=get_photogallery(self.site_id,lang);context[_W]=get_relatedlink(self.site_id,lang);context[_X]=get_socialmedia(self.site_id);context[_o]=get_location(self.site_id,lang);context[_O]=get_slideshow(self.site_id,lang);context[_M]=get_base_url(self.request,1);print(_t,context[_M])
...
class ListView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super(ListView,self).get_context_data(*(args),**kwargs);print('enter list view');context[_g]='list';context[_L]=self.request.device[_L];active_page=get_translated_active_page(_U);menu=get_menu_caches(self.request,_V,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);parent_order=1;menu_footer1=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer1);parent_order=2;menu_footer2=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer2);context[_d]=get_menu_group(self.site_id);slug=self.kwargs[_S]
		if not slug:raise Http404(_u%(self.request.get_host(),_k))
		agency=get_agency_info(self.site_id);context.update(agency);context[_N]=get_logo(self.site_id);lang=get_active_language_choices()[0];model_with_categories=[_C,_E,_K,_P,_i,_j];model_with_content=[_C,_E,_K,_P,_O,_b,_j,_J,_Y,_Z,_h,_l,_m,_a,_i];model_randomize=[_E,_K,_P,_Y,_Z,_h,_a];kind=self.kwargs[_I]
		if kind in model_with_content:context[_I]=kind
		else:raise Http404(_s)
		print('Kind = ',kind);context[_C]=get_announcement(self.site_id,lang,6);model=apps.get_model(_G,kind);latest_kind=_A;random_paint=_A;print('1',model_randomize)
		if kind in model_with_categories:context[_A3]=get_categories_list(self.site_id,lang,10,model);context[_A4]=get_tags_list(self.site_id,lang,10,model);latest_kind=get_latest_model(self.site_id,lang,10,model,kind,slug);random_paint=get_related_model(self.site_id,lang,10,model,kind,slug)
		...
		context[_J]=get_photogallery(self.site_id,lang);context[_W]=get_relatedlink(self.site_id,lang);context[_X]=get_socialmedia(self.site_id);context[_o]=get_location(self.site_id,lang);context[_M]=get_base_url(self.request,1);context[_O]=get_slideshow(self.site_id,lang);return context
class DescriptionView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(DescriptionView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_U);menu=get_menu_caches(self.request,_V,self.site_id,active_page,kinds=1);context.update(menu);context[_d]=get_menu_group(self.site_id);agency=get_agency_info(self.site_id);context.update(agency);context[_N]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_C]=get_announcement(self.site_id,lang,6);model=apps.get_model(_G,_C);context[_w]=get_latest_model(self.site_id,lang,5,model,_C);model=apps.get_model(_G,_E);context[_x]=get_latest_model(self.site_id,lang,5,model,_E);context[_J]=get_photogallery(self.site_id,lang);context[_W]=get_relatedlink(self.site_id,lang);context[_X]=get_socialmedia(self.site_id);context[_M]=get_base_url(self.request);return context
class GreetingView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(GreetingView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_U);menu=get_menu_caches(self.request,_V,self.site_id,active_page,1);context.update(menu);context[_d]=get_menu_group(self.site_id);agency=get_agency_info(self.site_id);context.update(agency);context[_N]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_C]=get_announcement(self.site_id,lang,6);context[_b]=get_greeting(self.site_id,lang);model=apps.get_model(_G,_C);context[_w]=get_latest_model(self.site_id,lang,5,model,_C);model=apps.get_model(_G,_E);context[_x]=get_latest_model(self.site_id,lang,5,model,_E);context[_J]=get_photogallery(self.site_id,lang);context[_W]=get_relatedlink(self.site_id,lang);context[_X]=get_socialmedia(self.site_id);context[_M]=get_base_url(self.request);return context
class BookingView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(BookingView,self).get_context_data(*(args),**kwargs);context[_g]='detail';context[_L]=self.request.device[_L];active_page=get_translated_active_page(_U);menu=get_menu_caches(self.request,_V,self.site_id,active_page,1);context.update(menu);parent_order=1;menu_footer1=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer1);parent_order=2;menu_footer2=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer2);context[_d]=get_menu_group(self.site_id);agency=get_agency_info(self.site_id);context.update(agency);context[_N]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_O]=get_slideshow(self.site_id,lang);context[_C]=get_announcement(self.site_id,lang,6);context[_b]=get_greeting(self.site_id,lang);model=apps.get_model(_G,_C);context[_w]=get_latest_model(self.site_id,lang,5,model,_C);model=apps.get_model(_G,_E);context[_x]=get_latest_model(self.site_id,lang,5,model,_E);context[_v]='Booking';context[_J]=get_photogallery(self.site_id,lang);context[_W]=get_relatedlink(self.site_id,lang);context[_X]=get_socialmedia(self.site_id);context[_M]=get_base_url(self.request);return context
...
def get_autoheadline(site_id,lang):
	model_name='Auto Headline';is_refresh=_F;initial_date=timezone.now();expired_in=1;most_view_within=7;max_data=15;model_list=ModelList.objects.filter(name=model_name)[:1]
	...
	if not tmp_log:is_refresh=_H;Log.objects.create(site_id=site_id,model_list=model_list,expired=initial_date+datetime.timedelta(days=expired_in))
		...
		if is_refresh:post=tmp_log[0];post.is_need_refresh=_F;post.save()
	if not is_refresh:
		if tmp_log:
	...
	if is_refresh:
		...
		for i in obj_announcement:
			...
			max_data-=1
		subquery_foto=get_photo(_K);obj_article=Article.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED,is_header_text=_F,created_at__gte=count_days).annotate(file_path=subquery_foto)[:5]
		for i in obj_article:
			...
			if i.file_path:destination=copy_image(i.file_path);Photo.objects.create(content_object=obj,file_path=destination)
			max_data-=1
		subquery_foto=get_photo(_E);obj_news=News.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED,is_header_text=_F,created_at__gte=count_days).annotate(file_path=subquery_foto)[:10]
		for i in obj_news:
			...
			max_data-=1
			if max_data<=0:break
	subquery_foto=get_photo(_A2);obj=AutoHeadline.objects.filter(site_id=site_id).annotate(file_path=subquery_foto).order_by(_I,_T)
	for i in obj:i.created_at=get_natural_datetime(i.created_at)
	return obj
def get_article_notes(site_id,lang):
	subquery_foto=get_photo(_K);obj=Article.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED,is_header_text=_F).annotate(file_path=subquery_foto).order_by(_T)[:5]
	for i in obj:i.created_at=get_natural_datetime(i.created_at)
	return obj
def get_flash_news(site_id,lang):
	subquery_foto=get_photo(_E);obj=News.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED,is_header_text=_F).annotate(file_path=subquery_foto).order_by(_T)[:6]
	for i in obj:i.created_at=get_natural_datetime(i.created_at)
	return obj