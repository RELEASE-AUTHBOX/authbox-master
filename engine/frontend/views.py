...
def get_calendar_ajax(request,year,month):
	...
	return JsonResponse(res,safe=_L)
...
def get_logo(site_id,max_data=1):
	subquery_foto=get_photo(_M);logo=Logo.objects.filter(site_id=site_id).values(_j).annotate(file_path=subquery_foto)[:max_data]
	if logo:return logo
	return _A
...
def get_statistic(site_id,is_cache=_L):
...
def get_announcement(site_id,lang,max_data=3,max_words=20):
	...
	return obj
def get_slideshow(site_id,lang,max_data=5,is_random=_L):
	subquery_foto=get_photo(_N);obj=SlideShow.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_C)[:max_data]
...
def get_offers(site_id,lang,exclude_id=[],is_header_text=_A,is_shuffle=_A):
	...
	else:obj=Offers.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).exclude(id__in=exclude_id).exclude(is_header_text=_G).annotate(file_path=subquery_foto).order_by(_B,_Q)[:10]
	return obj
...
def get_events(site_id,lang,max_data=3):subquery_foto=get_photo(_O);return Events.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_B,_C)[:max_data]
def get_photogallery(site_id,lang,max_data=16):subquery_foto=get_photo(_H);return PhotoGallery.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_B,_Q)[:max_data]
...
def get_article(site_id,lang,max_data=3,max_words=20):
	subquery_foto=get_photo(_P);obj=Article.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_B,_C)[:max_data]
	for i in obj:
		if not i.is_header_text:i.content=Truncator(strip_tags(i.content)).words(max_words);i.created_at=get_natural_datetime(i.created_at)
	return obj
...
def get_categories_list(site_id,lang,max_data,model):
	...
	if obj:
		...
		categories_list=list(obj);categories_all={A:0,_m:all_data,_j:'All',_R:_q};categories_list.insert(0,categories_all);return categories_list
def get_tags_list(site_id,lang,max_data,model):
	...
	if obj:
		all_data=0
		for i in obj:all_data+=i[_m]
		tags_list=list(obj);tags_all={'tags_id':0,_m:all_data,_j:'All',_R:_q};tags_list.insert(0,tags_all);return tags_list
...
def get_related_model(site_id,lang,max_data,model,kind,exclude_slug=_A):
	subquery_foto=get_photo(kind)
	if exclude_slug:qs=model.objects.translated(lang).filter(site_id=site_id,is_header_text=_L).exclude(slug=exclude_slug)
	else:qs=model.objects.translated(lang).filter(site_id=site_id,is_header_text=_L)
...
def get_content_list(site_id,lang,model,kind,slug):
	...
	field_is_header_text_exists=_L
	for field in model._meta.get_fields():
		if field.name=='is_header_text':field_is_header_text_exists=_G;break
	subquery_foto=get_photo(kind)
	if slug==_q:
		if field_is_header_text_exists:return model.objects.translated(lang).filter(site_id=site_id,is_header_text=_L).annotate(file_path=subquery_foto).order_by(A)
		...
		if categories:
			if field_is_header_text_exists:return model.objects.translated(lang).filter(site_id=site_id,categories_id=categories.id,is_header_text=_L).annotate(file_path=subquery_foto).order_by(A)
...
class IndexView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super().get_context_data(*(args),**kwargs);context[_e]='index';context['menu_justify']='right';context['agency_meta']=get_agency_meta(self.request,self.site_id);context[_I]=self.request.device[_I];print('OptSettingName',OptSettingName)
		for i in OptSettingName:
		...
		active_page=get_translated_active_page(_T);menu=get_menu_caches(self.request,_U,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);parent_order=1;menu_footer1=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer1);parent_order=2;menu_footer2=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer2);print('menu_footer1=',menu_footer1);print('menu_footer2=',menu_footer2);agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_G);context.update(statistic);context[_M]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_y]=get_banner(self.site_id);context[_D]=get_announcement(self.site_id,lang,max_data=10);context[_N]=get_slideshow(self.site_id,lang,max_data=10);context['dailyalert']=get_dailyalert(self.site_id,lang,max_data=10);context[_f]=get_howitworks(self.site_id,lang,max_data=10);context[_k]=get_aboutus(self.site_id,lang,max_data=5);context[_l]=get_testimony(self.site_id,lang,max_data=10);context[_Z]=get_product(self.site_id,lang,max_data=10);context['whyus']=get_whyus(self.site_id,lang,max_data=10);context[_X]=get_fasilities(self.site_id,lang);context[_Y]=get_offers(self.site_id,lang);context[_a]=get_greeting(self.site_id,lang);context[_O]=get_events(self.site_id,lang);context[_H]=get_photogallery(self.site_id,lang);print('!!! photo gallery !!! ====',context[_H]);context[_z]=get_videogallery(self.site_id,lang);context[_V]=get_relatedlink(self.site_id,lang);context[_F]=get_news(self.site_id,lang,max_data=10);context[_P]=get_article(self.site_id,lang,max_data=10);context[_g]=get_document(self.site_id,lang,max_data=10);context[_W]=get_socialmedia(self.site_id,max_data=10);context[_n]=get_location(self.site_id,lang);context[_J]=get_base_url(self.request);print(_s,context[_J]);og=get_og(self.site_id);context.update(og);return context
class CheckOutView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		B='footer_menu_2';A='footer_menu_1';context=super().get_context_data(*(args),**kwargs);context[_e]='checkout';context[_I]=self.request.device[_I];active_page=get_translated_active_page(_T);menu=get_menu_caches(self.request,_U,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);menu_footer1=get_menu_caches_footer(self.request,A,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=A);context.update(menu_footer1);menu_footer2=get_menu_caches_footer(self.request,B,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=B);context.update(menu_footer2);context[_c]=get_menu_group(self.site_id);slug=self.kwargs[_R]
		if not slug:raise Http404(_t%(self.request.get_host(),_i))
		kind=self.kwargs[_K];context[_K]=kind;model=apps.get_model(_E,kind);agency=get_agency_info(self.site_id);context.update(agency);context[_M]=get_logo(self.site_id);lang=get_active_language_choices()[0];content_detail=get_content_detail(self.site_id,lang,model,kind,slug);context[_u]=content_detail;context[_V]=get_relatedlink(self.site_id,lang);context[_W]=get_socialmedia(self.site_id);context[_N]=get_slideshow(self.site_id,lang);context[_J]=get_base_url(self.request,1);print(_s,context[_J]);return context
class DetailView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		B='random_paint_kind';A='latest_kind_kind';context=super(DetailView,self).get_context_data(*(args),**kwargs);context[_e]='detail';context[_I]=self.request.device[_I];active_page=get_translated_active_page(_T);menu=get_menu_caches(self.request,_U,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);parent_order=1;menu_footer1=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer1);parent_order=2;menu_footer2=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer2);context[_c]=get_menu_group(self.site_id);slug=self.kwargs[_R]
		if not slug:raise Http404(_t%(self.request.get_host(),_i))
		agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_G);context.update(statistic);context[_M]=get_logo(self.site_id);lang=get_active_language_choices()[0];model_with_categories=[_D,_F,_P,_O,_g,_h];model_with_content=[_D,_F,_P,_O,_N,_a,_h,_H,_X,_Y,_f,_k,_l,_Z,_g];model_randomize=[_F,_P,_O,_X,_Y,_f,_Z,_h];kind=self.kwargs[_K];print('kind detail',kind)
		...
		if not latest_kind:
			...
			for i in model_randomize:
				print(_d,i);tmp_kind=i;tmp_model=apps.get_model(_E,i);latest_kind=get_latest_model(self.site_id,lang,10,tmp_model,i)
		...
		if not random_paint:
			random.shuffle(model_randomize)
			for i in model_randomize:
				print(_d,i);tmp_kind=i;tmp_model=apps.get_model(_E,i);random_paint=get_related_model(self.site_id,lang,4,tmp_model,i)
		...
		for i in model_randomize:
			print(_d,i);tmp_model=apps.get_model(_E,i);random_model=get_model_content(self.site_id,lang,tmp_model,i,10)
			if random_model:break
		context[_A5]=random_model;content_detail=get_content_detail(self.site_id,lang,model,kind,slug);context[_u]=content_detail;hit_count=HitCount.objects.get_for_object(content_detail);hit_count_response=HitCountMixin.hit_count(self.request,hit_count);context[_H]=get_photogallery(self.site_id,lang);context[_V]=get_relatedlink(self.site_id,lang);context[_W]=get_socialmedia(self.site_id);context[_n]=get_location(self.site_id,lang);context[_N]=get_slideshow(self.site_id,lang);context[_J]=get_base_url(self.request,1);print(_s,context[_J])
...
class ListView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super(ListView,self).get_context_data(*(args),**kwargs);print('enter list view');context[_e]='list';context[_I]=self.request.device[_I];active_page=get_translated_active_page(_T);menu=get_menu_caches(self.request,_U,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);parent_order=1;menu_footer1=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer1);parent_order=2;menu_footer2=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer2);context[_c]=get_menu_group(self.site_id);slug=self.kwargs[_R]
		if not slug:raise Http404(_t%(self.request.get_host(),_i))
		agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_G);context.update(statistic);context[_M]=get_logo(self.site_id);lang=get_active_language_choices()[0];model_with_categories=[_D,_F,_P,_O,_g,_h];model_with_content=[_D,_F,_P,_O,_N,_a,_h,_H,_X,_Y,_f,_k,_l,_Z,_g];model_randomize=[_F,_P,_O,_X,_Y,_f,_Z];kind=self.kwargs[_K]
		if kind in model_with_content:context[_K]=kind
		...
		context[_H]=get_photogallery(self.site_id,lang);context[_V]=get_relatedlink(self.site_id,lang);context[_W]=get_socialmedia(self.site_id);context[_n]=get_location(self.site_id,lang);context[_J]=get_base_url(self.request,1);context[_N]=get_slideshow(self.site_id,lang);return context
class DescriptionView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(DescriptionView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_T);menu=get_menu_caches(self.request,_U,self.site_id,active_page,kinds=1);context.update(menu);context[_c]=get_menu_group(self.site_id);agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_G);context.update(statistic);context[_M]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_D]=get_announcement(self.site_id,lang,6);model=apps.get_model(_E,_D);context[_v]=get_latest_model(self.site_id,lang,5,model,_D);model=apps.get_model(_E,_F);context[_w]=get_latest_model(self.site_id,lang,5,model,_F);context[_H]=get_photogallery(self.site_id,lang);context[_V]=get_relatedlink(self.site_id,lang);context[_W]=get_socialmedia(self.site_id);context[_J]=get_base_url(self.request);return context
class GreetingView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(GreetingView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_T);menu=get_menu_caches(self.request,_U,self.site_id,active_page,1);context.update(menu);context[_c]=get_menu_group(self.site_id);agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_G);context.update(statistic);context[_M]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_D]=get_announcement(self.site_id,lang,6);context[_a]=get_greeting(self.site_id,lang);model=apps.get_model(_E,_D);context[_v]=get_latest_model(self.site_id,lang,5,model,_D);model=apps.get_model(_E,_F);context[_w]=get_latest_model(self.site_id,lang,5,model,_F);context[_H]=get_photogallery(self.site_id,lang);context[_V]=get_relatedlink(self.site_id,lang);context[_W]=get_socialmedia(self.site_id);context[_J]=get_base_url(self.request);return context
class BookingView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(BookingView,self).get_context_data(*(args),**kwargs);context[_e]='detail';context[_I]=self.request.device[_I];active_page=get_translated_active_page(_T);menu=get_menu_caches(self.request,_U,self.site_id,active_page,1);context.update(menu);parent_order=1;menu_footer1=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer1);parent_order=2;menu_footer2=get_menu_caches_footer2(self.request,f"footer_menu_{parent_order}",self.site_id,active_page,kinds=1,exclude_menu=1,parent_order=parent_order);context.update(menu_footer2);context[_c]=get_menu_group(self.site_id);agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_G);context.update(statistic);context[_M]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_N]=get_slideshow(self.site_id,lang);context[_D]=get_announcement(self.site_id,lang,6);context[_a]=get_greeting(self.site_id,lang);model=apps.get_model(_E,_D);context[_v]=get_latest_model(self.site_id,lang,5,model,_D);model=apps.get_model(_E,_F);context[_w]=get_latest_model(self.site_id,lang,5,model,_F);context[_u]='Booking';context[_H]=get_photogallery(self.site_id,lang);context[_V]=get_relatedlink(self.site_id,lang);context[_W]=get_socialmedia(self.site_id);context[_J]=get_base_url(self.request);return context