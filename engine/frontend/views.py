...
def get_calendar_ajax(request,year,month):
	B='start';A='get_calendar_ajax';site_id=5;res=[];CLIENT_SECRET_FILE='credentials/narvik/credentials.json';API_NAME='calendar';API_VERSION='v3';SCOPES=['https://www.googleapis.com/auth/calendar'];service=create_service(CLIENT_SECRET_FILE,API_NAME,API_VERSION,SCOPES);calendar_id='suratiwan03@gmail.com';timeZone='Asia/Makassar';TZA=pytz.timezone(timeZone);res=[];gc=GoogleCalendar.objects.filter(calendar_id=calendar_id)[:1]
	if gc:
		gcd=GoogleCalendarDetail.objects.filter(cal_year=2024,cal_month=8,site_id=site_id,google_calendar=gc).order_by(B)
		for i in gcd:tmp={'title':i.summary,B:i.start.astimezone(TZA).isoformat(),'end':i.end.astimezone(TZA).isoformat(),'desc':i.description};res.append(tmp)
...
class ListView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super(ListView,self).get_context_data(*args,**kwargs);context[_m]='list';context[_N]=self.request.device[_N];active_page=get_translated_active_page(_T);menu=get_menu_caches(self.request,_U,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);menu_footer1=get_menu_caches_footer(self.request,_c,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=_c);context.update(menu_footer1);menu_footer2=get_menu_caches_footer(self.request,_d,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=_d);context.update(menu_footer2);context[_e]=get_menu_group(self.site_id);slug=self.kwargs[_R]
		if not slug:raise Http404(_r%(self.request.get_host(),_i))
		agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_H);context.update(statistic);context[_J]=get_logo(self.site_id);lang=get_active_language_choices()[0];model_with_categories=[_D,_F,_M,_L,_h,_n];model_with_content=[_D,_F,_M,_L,_K,_a,_n,_G,_X,_Y,_g,_j,_k,_Z,_h];model_randomize=[_F,_M,_L,_X,_Y,_g,_Z];kind=self.kwargs[_O]
		if kind in model_with_content:context[_O]=kind
