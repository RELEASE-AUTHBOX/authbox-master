...
def get_calendar_ajax(request,year,month):
	print('get_calendar_ajax - request',request);site_id=get_site_id_front(request);res=[];calendar_id=get_calendar_id(site_id);print('calendar_id',calendar_id);timeZone=_A4;bg_color=['rgb(220, 235, 252)','rgb(173, 209, 245)','rgb(129, 180, 237)','rgb(38, 101, 167)','rgb(0, 116, 217)'];bg_cal_name=[];TZA=pytz.timezone(timeZone);res=[];gc=GoogleCalendar.objects.filter(calendar_id=calendar_id)[:1]
	if gc:
...
def get_fasilities(site_id,lang,exclude_id=[],is_header_text=_A,is_shuffle=_A):
	...
	if is_header_text is _A:print('isheadertext1',is_header_text);obj=Fasilities.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).exclude(id__in=exclude_id).annotate(file_path=subquery_foto).order_by(_D,_S)[:10];print(obj)
	else:print('isheadertext2',is_header_text);obj=Fasilities.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).exclude(id__in=exclude_id).exclude(is_header_text=is_header_text).annotate(file_path=subquery_foto).order_by(_D,_S)[:10]
	obj=list(obj)
...
class IndexView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super().get_context_data(*(args),**kwargs);context[_g]='index';context['menu_justify']='right';context[_A8]=get_agency_meta(self.request,self.site_id);context[_M]=self.request.device[_M];print('OptSettingName',OptSettingName)
		for i in OptSettingName:
...
class DetailView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		if not slug:raise Http404(_A0%(self.request.get_host(),_k))
		agency=get_agency_info(self.site_id);context.update(agency);context[_A8]=get_agency_meta(self.request,self.site_id);context[_O]=get_logo(self.site_id);context[_u]=get_logo_pos(self.site_id,OptLogoSettingPos.TOP_NORMAL);context[_v]=get_logo_pos(self.site_id,OptLogoSettingPos.TOP_DARK);context[_w]=get_logo_pos(self.site_id,OptLogoSettingPos.BOTTOM_NORMAL);context[_x]=get_logo_pos(self.site_id,OptLogoSettingPos.BOTTOM_DARK);context[_y]=get_format_date();lang=get_active_language_choices()[0];model_with_categories=[_C,_E,_I,_Q,_i,_j];model_with_content=[_C,_E,_I,_Q,_P,_b,_j,_L,_Y,_Z,_h,_m,_n,_a,_i];model_randomize=[_E,_I,_Q,_Y,_Z,_h,_a,_j];kind=self.kwargs[_J];print('kind detail',kind)
		if kind in model_with_content:context[_J]=kind
		context[_C]=get_announcement(self.site_id,lang,6);model=apps.get_model(_H,kind);latest_kind=_A;random_paint=_A;print('1',model_randomize)
		if kind in model_with_categories:context[_AA]=get_categories_list(self.site_id,lang,10,model);context[_AB]=get_tags_list(self.site_id,lang,10,model);latest_kind=get_latest_model(self.site_id,lang,10,model,kind,slug);random_paint=get_related_model(self.site_id,lang,10,model,kind,slug)
		print(_J,kind);idx=0
		...
		if not latest_kind:
			...
			for i in model_randomize:
				print(_e,i);tmp_kind=i;tmp_model=apps.get_model(_H,i);latest_kind=get_latest_model(self.site_id,lang,10,tmp_model,i)
				if latest_kind:break
		context[_AC]=latest_kind;context[A]=tmp_kind;idx=0
		for i in model_randomize:
		...
		if not random_paint:
			random.shuffle(model_randomize)
			for i in model_randomize:
				print(_e,i);tmp_kind=i;tmp_model=apps.get_model(_H,i);random_paint=get_related_model(self.site_id,lang,4,tmp_model,i)
				if random_paint:break
		context[_AD]=random_paint;context[B]=tmp_kind;idx=0
		for i in model_randomize:
			if i==tmp_kind:model_randomize.pop(idx);break
			idx+=1
		print('4',model_randomize);random_model=_A;random.shuffle(model_randomize)
		for i in model_randomize:
			print(_e,i);tmp_model=apps.get_model(_H,i);random_model=get_model_content(self.site_id,lang,tmp_model,i,10)
			if random_model:break
		context[_AE]=random_model;content_detail=get_content_detail(self.site_id,lang,model,kind,slug);context[_A1]=content_detail;context[_L]=get_photogallery(self.site_id,lang);context[_W]=get_relatedlink(self.site_id,lang);context[_X]=get_socialmedia(self.site_id);context[_p]=get_location(self.site_id,lang);context[_P]=get_slideshow(self.site_id,lang);context[_N]=get_base_url(self.request,1);print(_z,context[_N])
...
class ListView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		...
		print('Kind = ',kind);context[_C]=get_announcement(self.site_id,lang,6);model=apps.get_model(_H,kind);latest_kind=_A;random_paint=_A;print('1',model_randomize)
		if kind in model_with_categories:context[_AA]=get_categories_list(self.site_id,lang,10,model);context[_AB]=get_tags_list(self.site_id,lang,10,model);latest_kind=get_latest_model(self.site_id,lang,10,model,kind,slug);random_paint=get_related_model(self.site_id,lang,10,model,kind,slug)
		...
		if not random_paint:
			...
			for i in model_randomize:
				print(_e,i);tmp_kind=i;tmp_model=apps.get_model(_H,i);random_paint=get_related_model(self.site_id,lang,4,tmp_model,i)
				if random_paint:break
		context[_AD]=random_paint;idx=0
		for i in model_randomize:
			if i==tmp_kind:model_randomize.pop(idx);break
			idx+=1
		print('4',model_randomize);random_model=_A;random.shuffle(model_randomize)
		for i in model_randomize:
			print(_e,i);tmp_model=apps.get_model(_H,i);random_model=get_model_content(self.site_id,lang,tmp_model,i,10)
			if random_model:break
		context[_AE]=random_model;content_list=get_content_list(self.site_id,lang,model,kind,slug)
...
class BookingView(TemplateView):
	...
	def post(self,request,*args,**kwargs):
		...
		if not site:print('Site Not Found!');template=get_template(self.site_id);self.template_name=template+_q;return self.render_to_response(context)
		...
		if not cal:print('Calendar Not Found!');template=get_template(self.site_id);self.template_name=template+_q;return self.render_to_response(context)
...
def get_autoheadline(site_id,lang):
	...
	if is_refresh:
		AutoHeadline.objects.filter(site_id=site_id,is_editable=_G).delete();count_days=timezone.now()-datetime.timedelta(days=most_view_within);count_days=count_days.replace(hour=0,minute=0,second=0,microsecond=0);print('count_days',count_days);subquery_foto=get_photo(_C);obj_announcement=Announcement.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED,is_header_text=_B,created_at__gte=count_days).annotate(file_path=subquery_foto)[:3]
		for i in obj_announcement:
			...
			max_data-=1
		subquery_foto=get_photo(_I);obj_article=Article.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED,is_header_text=_B,created_at__gte=count_days).annotate(file_path=subquery_foto)[:5];print(_I,obj_article)
		for i in obj_article:
			...
			if i.file_path:destination=copy_image(i.file_path);Photo.objects.create(content_object=obj,file_path=destination)
			max_data-=1
		subquery_foto=get_photo(_E);obj_news=News.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED,is_header_text=_B,created_at__gte=count_days).annotate(file_path=subquery_foto)[:10];print(_E,obj_news)
		for i in obj_news:
			...
			max_data-=1
			if max_data<=0:break
	subquery_foto=get_photo(_A9);obj=AutoHeadline.objects.filter(site_id=site_id).annotate(file_path=subquery_foto).order_by(_J,_K)
	for i in obj:i.created_at=get_natural_datetime(i.created_at)
	return obj
