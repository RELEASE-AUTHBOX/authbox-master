...
def get_calendar_ajax(request,year,month):A='get_calendar_ajax';res=sync_calendar_all(request,year,month);return JsonResponse(res,safe=_R)
...
def get_fasilities(site_id,lang,exclude_id=[],is_header_text=_A,is_shuffle=_A):
	...
	if is_header_text is _A:obj=Fasilities.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).exclude(id__in=exclude_id).annotate(file_path=subquery_foto).order_by(_B,_S)[:10];
	else:obj=Fasilities.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).exclude(id__in=exclude_id).exclude(is_header_text=is_header_text).annotate(file_path=subquery_foto).order_by(_B,_S)[:10]
	obj=list(obj)
...
class IndexView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super().get_context_data(*(args),**kwargs);context[_m]='index';context['menu_justify']='right';context['agency_meta']=get_agency_meta(self.request,self.site_id);context[_N]=self.request.device[_N];
		for i in OptSettingName:
...
class ListView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super(ListView,self).get_context_data(*(args),**kwargs);context[_m]='list';context[_N]=self.request.device[_N];active_page=get_translated_active_page(_V);menu=get_menu_caches(self.request,_W,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);menu_footer1=get_menu_caches_footer(self.request,_O,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=_O);context.update(menu_footer1);menu_footer2=get_menu_caches_footer(self.request,_P,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=_P);context.update(menu_footer2);context[_e]=get_menu_group(self.site_id);slug=self.kwargs[_T]
		if not slug:raise Http404(_r%(self.request.get_host(),_i))
		agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_G);context.update(statistic);context[_J]=get_logo(self.site_id);lang=get_active_language_choices()[0];model_with_categories=[_D,_F,_M,_L,_h,_n];model_with_content=[_D,_F,_M,_L,_K,_c,_n,_H,_Z,_a,_g,_j,_k,_b,_h];model_randomize=[_F,_M,_L,_Z,_a,_g,_b];kind=self.kwargs[_Q]
		if kind in model_with_content:context[_Q]=kind
