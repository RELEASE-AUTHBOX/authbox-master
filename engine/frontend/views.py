...
_y='latest_kind'
_x='categories_list'
_w="Menu Group '%s' belum terdaftar, silahkan daftar di halaman <a href='%s'>admin</a>"
_v='translations__name'
_u='banner'
_t='latest_news'
...
def get_model_content(site_id,lang,model,kind,max_data):subquery_foto=get_photo(kind);tmp=model.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_D,_B)[:max_data];print('get_model_content',tmp);return tmp
def get_banner(site_id):subquery_foto=get_photo(_u);return Banner.objects.filter(site_id=site_id).annotate(file_path=subquery_foto)[:5]
...
def get_events(site_id,lang):subquery_foto=get_photo(_M);return Events.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_B)[:7]
...
def get_article(site_id,lang):subquery_foto=get_photo(_N);return Article.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_D,_B)[:6]
...
def get_categories_list(site_id,lang,max_data,model):
	A='categories_id';subquery=Subquery(Categories.objects.translated(lang).filter(id=OuterRef(A)).values(_v)[:1]);subquery_slug=Subquery(Categories.objects.translated(lang).filter(id=OuterRef(A)).values(_O)[:1]);categories_list=[];obj=model.objects.filter(site_id=site_id).values(A).annotate(count=Count(A)).annotate(name=subquery).annotate(slug=subquery_slug).order_by(A)[:max_data]
	if obj:
		all_data=0
		for i in obj:all_data+=i[_d]
		categories_list=list(obj);categories_all={A:0,_d:all_data,_i:'All',_O:_l};categories_list.insert(0,categories_all);return categories_list
def get_tags_list(site_id,lang,max_data,model):
	site_name={f"{model.__name__.lower()}__site":f"{site_id}"};val=f"{model.__name__.lower()}__tags";subquery=Subquery(Tags.objects.translated(lang).filter(id=OuterRef(val)).values(_v)[:1]);subquery_slug=Subquery(Tags.objects.translated(lang).filter(id=OuterRef(val)).values(_O)[:1]);tags_list=[];obj=model.tags.through.objects.filter(**site_name).values(val).annotate(count=Count(val)).annotate(name=subquery).annotate(slug=subquery_slug).order_by(val)[:max_data]
	if obj:
		all_data=0
		for i in obj:all_data+=i[_d]
		tags_list=list(obj);tags_all={'tags_id':0,_d:all_data,_i:'All',_O:_l};tags_list.insert(0,tags_all);return tags_list
...
def get_related_model(site_id,lang,max_data,model,kind,exclude_slug=_A):
	...
	if qs:random_paint=get_random_items(qs,max_data);random_paint=random_paint.annotate(file_path=subquery_foto);header_text=model.objects.translated(lang).filter(site_id=site_id,is_header_text=_H).annotate(file_path=subquery_foto);return (header_text|random_paint).order_by(_D)
...
class IndexView(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		...
		if not service_type:raise Http404("service belum terdaftar, silahkan daftar di halaman <a href='%s'>admin</a>"%_P)
	...
	def get_context_data(self,*args,**kwargs):context=super(IndexView,self).get_context_data(*(args),**kwargs);context[_m]='index';active_page=get_translated_active_page(_Q);menu=get_menu_caches(self.request,_R,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);menu_footer1=get_menu_caches_footer(self.request,_n,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=_o);context.update(menu_footer1);menu_footer2=get_menu_caches_footer(self.request,_p,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=_q);context.update(menu_footer2);agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_H);context.update(statistic);context[_I]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_u]=get_banner(self.site_id);context[_C]=get_announcement(self.site_id,lang,4);context[_U]=get_slideshow(self.site_id,lang);context['dailyalert']=get_dailyalert(self.site_id,lang);context[_r]=get_howitworks(self.site_id,lang);context[_j]=get_aboutus(self.site_id,lang);context[_k]=get_testimony(self.site_id,lang);context[_b]=get_product(self.site_id,lang);context['whyus']=get_whyus(self.site_id,lang);context[_Z]=get_fasilities(self.site_id,lang);context[_a]=get_offers(self.site_id,lang);context[_K]=get_greeting(self.site_id,lang);context[_M]=get_events(self.site_id,lang);context[_G]=get_photogallery(self.site_id,lang);context[_c]=get_videogallery(self.site_id,lang);context[_S]=get_relatedlink(self.site_id,lang);context[_E]=get_news(self.site_id,lang);context[_N]=get_article(self.site_id,lang);context[_f]=get_document(self.site_id,lang);context[_T]=get_socialmedia(self.site_id);context['location']=get_location(self.site_id,lang);context[_L]=get_base_url(self.request);return context
class DetailView(TemplateView):
	site_id=_A
	def get(self,request,*args,**kwargs):
		self.site_id=get_site_id_front(request);service=service_exists(request)
		if not service:raise Http404(_V%(request.get_host(),_P))
	...
	def get_context_data(self,*args,**kwargs):
		A='proses';context=super(DetailView,self).get_context_data(*(args),**kwargs);context[_m]='detail';active_page=get_translated_active_page(_Q);menu=get_menu_caches(self.request,_R,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);menu_footer1=get_menu_caches_footer(self.request,_n,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=_o);context.update(menu_footer1);menu_footer2=get_menu_caches_footer(self.request,_p,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=_q);context.update(menu_footer2);context[_W]=get_menu_group(self.site_id);slug=self.kwargs[_O]
		if not slug:raise Http404(_w%(self.request.get_host(),_h))
		agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_H);context.update(statistic);context[_I]=get_logo(self.site_id);lang=get_active_language_choices()[0];model_with_categories=[_C,_E,_N,_M,_f,_g];model_with_content=[_C,_E,_N,_M,_U,_K,_g,_G,_Z,_a,_r,_j,_k,_b,_f];model_randomize=[_E,_N,_M,_Z,_a,_r,_b];kind=self.kwargs[_X]
		...
		if kind in model_with_categories:context[_x]=get_categories_list(self.site_id,lang,10,model);context['tags_list']=get_tags_list(self.site_id,lang,10,model);latest_kind=get_latest_model(self.site_id,lang,4,model,kind,slug);random_paint=get_related_model(self.site_id,lang,4,model,kind,slug)
		print(_X,kind);idx=0
		...
		if not latest_kind:
			...
			for i in model_randomize:
				print(A,i);tmp_kind=i;tmp_model=apps.get_model(_F,i);latest_kind=get_latest_model(self.site_id,lang,4,tmp_model,i)
				if latest_kind:break
		context[_y]=latest_kind;idx=0
		for i in model_randomize:
		...
		if not random_paint:
			random.shuffle(model_randomize)
			for i in model_randomize:
				print(A,i);tmp_kind=i;tmp_model=apps.get_model(_F,i);random_paint=get_related_model(self.site_id,lang,4,tmp_model,i)
				if random_paint:break
		context[_z]=random_paint;idx=0
		for i in model_randomize:
			if i==tmp_kind:model_randomize.pop(idx);break
			idx+=1
		print('4',model_randomize);random_model=_A;random.shuffle(model_randomize)
		for i in model_randomize:
			print(A,i);tmp_model=apps.get_model(_F,i);random_model=get_model_content(self.site_id,lang,tmp_model,i,7)
			if random_model:break
		context['random_model']=random_model;content_detail=get_content_detail(self.site_id,lang,model,kind,slug);context['content_detail']=content_detail;hit_count=HitCount.objects.get_for_object(content_detail);hit_count_response=HitCountMixin.hit_count(self.request,hit_count);context[_G]=get_photogallery(self.site_id,lang);context[_S]=get_relatedlink(self.site_id,lang);context[_T]=get_socialmedia(self.site_id);context[_L]=get_base_url(self.request,1);print('baseurl',context[_L]);return context
class ListView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super(ListView,self).get_context_data(*(args),**kwargs);context[_m]='list';active_page=get_translated_active_page(_Q);menu=get_menu_caches(self.request,_R,self.site_id,active_page,kinds=1,exclude_menu=0);context.update(menu);menu_footer1=get_menu_caches_footer(self.request,_n,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=_o);context.update(menu_footer1);menu_footer2=get_menu_caches_footer(self.request,_p,self.site_id,active_page,kinds=1,exclude_menu=1,parent_name=_q);context.update(menu_footer2);context[_W]=get_menu_group(self.site_id);slug=self.kwargs[_O]
		if not slug:raise Http404(_w%(self.request.get_host(),_h))
		agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_H);context.update(statistic);context[_I]=get_logo(self.site_id);lang=get_active_language_choices()[0];kinds=[_C,_E,_N,_M,_f,_K,_g,_U,_c,_G];kind=self.kwargs[_X]
		...
		if kind not in[_g,_K,_U,_c,_G]:context[_x]=get_categories_list(self.site_id,lang,10,model);context[_y]=get_latest_model(self.site_id,lang,3,model,kind,slug);context[_z]=get_related_model(self.site_id,lang,5,model,kind,slug)
		...
		context[_G]=get_photogallery(self.site_id,lang);context[_S]=get_relatedlink(self.site_id,lang);context[_T]=get_socialmedia(self.site_id);context[_L]=get_base_url(self.request);return context
class DescriptionView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(DescriptionView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Q);menu=get_menu_caches(self.request,_R,self.site_id,active_page,kinds=1);context.update(menu);context[_W]=get_menu_group(self.site_id);agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_H);context.update(statistic);context[_I]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_C]=get_announcement(self.site_id,lang,6);model=apps.get_model(_F,_C);context[_s]=get_latest_model(self.site_id,lang,3,model,_C);model=apps.get_model(_F,_E);context[_t]=get_latest_model(self.site_id,lang,3,model,_E);context[_G]=get_photogallery(self.site_id,lang);context[_S]=get_relatedlink(self.site_id,lang);context[_T]=get_socialmedia(self.site_id);context[_L]=get_base_url(self.request);return context
class GreetingView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(GreetingView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Q);menu=get_menu_caches(self.request,_R,self.site_id,active_page,1);context.update(menu);context[_W]=get_menu_group(self.site_id);agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_H);context.update(statistic);context[_I]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_C]=get_announcement(self.site_id,lang,6);context[_K]=get_greeting(self.site_id,lang);model=apps.get_model(_F,_C);context[_s]=get_latest_model(self.site_id,lang,3,model,_C);model=apps.get_model(_F,_E);context[_t]=get_latest_model(self.site_id,lang,3,model,_E);context[_G]=get_photogallery(self.site_id,lang);context[_S]=get_relatedlink(self.site_id,lang);context[_T]=get_socialmedia(self.site_id);context[_L]=get_base_url(self.request);return context
class BookingView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(BookingView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Q);menu=get_menu_caches(self.request,_R,self.site_id,active_page,1);context.update(menu);context[_W]=get_menu_group(self.site_id);agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_H);context.update(statistic);context[_I]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_C]=get_announcement(self.site_id,lang,6);context[_K]=get_greeting(self.site_id,lang);model=apps.get_model(_F,_C);context[_s]=get_latest_model(self.site_id,lang,3,model,_C);model=apps.get_model(_F,_E);context[_t]=get_latest_model(self.site_id,lang,3,model,_E);context[_G]=get_photogallery(self.site_id,lang);context[_S]=get_relatedlink(self.site_id,lang);context[_T]=get_socialmedia(self.site_id);context[_L]=get_base_url(self.request);return context