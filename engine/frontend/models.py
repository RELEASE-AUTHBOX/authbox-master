...
_k='menu_class_2'
_j='menu_class_1'
_i='token_calendar_v3.pickle'
_h='auto list page'
_g='auto related news'
_f='auto Popular'
_e='auto flash news'
_d='auto article'
_c='google calendar'
...
from core.models import ModelList,ModelListSetting,OptLogoSettingPos,OptPosition,Photo
...
class OptPriority(models.IntegerChoices):HIGH=1,_('High');MIDDLE=2,_(_T);LOW=3,_('Low')
class OptValueType(models.IntegerChoices):TEXT=1,_('text');ICON=2,_(_N);IMAGE=3,_('image');UL=4,_('unordered-list');NAV=5,_('nav-bar')
...
class News(BaseAbstractModel,BaseContentModel,TranslatableModel,HitCountMixin):
	translations=TranslatedFields(title=encrypt(models.CharField(_(_C),max_length=LEN_TITLE)),sub_title=encrypt(models.CharField(_(_F),max_length=LEN_SUB_TITLE,null=_A,blank=_A)),content=encrypt(CKEditor5Field(_(_H),blank=_A,null=_A,config_name=_G)));word_count=models.PositiveIntegerField(default=0,blank=_A,editable=_B);reading_time=models.PositiveIntegerField(default=0,blank=_A,editable=_B);is_header_text=models.BooleanField(default=_B);is_editor_choice=models.BooleanField(default=_B);hit_count_generic=GenericRelation(HitCount,object_id_field=_O,related_query_name=_P)
...
class Events(BaseAbstractModel,BaseContentModel,TranslatableModel):
	translations=TranslatedFields(title=encrypt(models.CharField(_(_C),max_length=LEN_TITLE)),sub_title=encrypt(models.CharField(_(_F),max_length=LEN_SUB_TITLE,null=_A,blank=_A)),content=encrypt(CKEditor5Field(_(_H),blank=_A,null=_A,config_name=_G)),location=encrypt(models.CharField(_(_X),max_length=255,null=_A,blank=_A)));word_count=models.PositiveIntegerField(default=0,blank=_A,editable=_B);reading_time=models.PositiveIntegerField(default=0,blank=_A,editable=_B);is_header_text=models.BooleanField(default=_B);date=models.DateField(_('date'));time=models.TimeField(_('time'))
...
class WhyUs(BaseAbstractModel,TranslatableModel):
	site=models.ForeignKey(Site,on_delete=models.CASCADE);admin=models.ForeignKey(User,on_delete=models.PROTECT);icon=models.CharField(_(_N),max_length=100);translations=TranslatedFields(title=models.CharField(_(_C),max_length=LEN_TITLE),sub_title=encrypt(models.CharField(_(_F),max_length=LEN_SUB_TITLE,null=_A,blank=_A)),description=models.CharField(_(_Y),max_length=500));is_header_text=models.BooleanField(default=_B);status=models.SmallIntegerField(choices=OptStatusPublish.choices,default=OptStatusPublish.PUBLISHED)
...
class AboutUs(BaseAbstractModel,BaseGalleryModel,TranslatableModel):
	translations=TranslatedFields(sub_title=encrypt(models.CharField(_(_F),max_length=LEN_TITLE)),title=encrypt(models.CharField(_(_C),max_length=LEN_SUB_TITLE)),content=encrypt(CKEditor5Field(_(_H),blank=_A,null=_A,config_name='extends_noimage')));photo=GenericRelation(Photo,verbose_name=_(_E),null=_A,blank=_A)
	def __str__(A):return f"{A.title}"
	def save(A,*B,**C):
		if not A.slug:A.slug=uuslug(A.title,instance=A,max_length=255)
		super().save(*B,**C)
class Testimony(BaseAbstractModel,TranslatableModel):
	translations=TranslatedFields(content=encrypt(CKEditor5Field(_(_H),blank=_A,null=_A,config_name=_G)));subtitle=encrypt(models.CharField(_(_a),max_length=LEN_SUB_TITLE));title=encrypt(models.CharField(_(_C),max_length=LEN_TITLE));is_header_text=models.BooleanField(default=_B);photo=GenericRelation(Photo,verbose_name=_(_E),null=_A,blank=_A);site=models.ForeignKey(Site,on_delete=models.CASCADE,verbose_name=_(_D));admin=models.ForeignKey(User,on_delete=models.PROTECT);status=models.SmallIntegerField(choices=OptStatusPublish.choices,default=OptStatusPublish.PUBLISHED)
	def __str__(A):return f"{A.title} - {A.subtitle}"
class Product(BaseAbstractModel,BaseGalleryModel,TranslatableModel):
	translations=TranslatedFields(name=encrypt(models.CharField(_(_Q),max_length=LEN_NAME)),title=encrypt(models.CharField(_(_C),max_length=LEN_TITLE)),sub_title=encrypt(models.CharField(_(_F),max_length=LEN_SUB_TITLE,null=_A,blank=_A)),content=encrypt(CKEditor5Field(_(_H),blank=_A,null=_A,config_name=_G)));is_header_text=models.BooleanField(default=_B);order_item=models.PositiveIntegerField(default=0);icon=models.CharField(max_length=100,null=_A,blank=_A);photo=GenericRelation(Photo,verbose_name=_(_E),null=_A,blank=_A);price=MoneyField(max_digits=10,decimal_places=2,default=0,default_currency='IDR')
	def __str__(A):return f"{A.name}"
	def save(A,*C,**D):
...
class Cart(BaseAbstractModel):
	product=models.ForeignKey(Product,on_delete=models.CASCADE,verbose_name=_(_b));qty=models.PositiveIntegerField(default=1,blank=_A,editable=_B);site=models.ForeignKey(Site,on_delete=models.CASCADE,verbose_name=_(_D));admin=models.ForeignKey(User,on_delete=models.PROTECT);status=models.SmallIntegerField(choices=OptStatusPublish.choices,default=OptStatusPublish.PUBLISHED)
...
class RelatedLink(BaseAbstractModel,TranslatableModel):
	site=models.ForeignKey(Site,on_delete=models.CASCADE,blank=_A,verbose_name=_(_D));link=encrypt(models.URLField(_(_L),max_length=255));translations=TranslatedFields(name=encrypt(models.CharField(_(_Q),max_length=LEN_NAME)));status=models.SmallIntegerField(choices=OptStatusPublish.choices,default=OptStatusPublish.PUBLISHED)
	def __str__(A):return f"{A.name}"
class Document(BaseAbstractModel,BaseContentModel,TranslatableModel):
	file_path_doc=models.FileField(verbose_name=_('file path Document'),upload_to='temp_doc');translations=TranslatedFields(name=encrypt(models.CharField(_(_Q),max_length=LEN_NAME)),content=encrypt(CKEditor5Field(_(_H),blank=_A,null=_A,config_name=_G)));word_count=models.PositiveIntegerField(default=0,blank=_A,editable=_B);reading_time=models.PositiveIntegerField(default=0,blank=_A,editable=_B);is_header_text=models.BooleanField(default=_B);size=models.BigIntegerField(_('size'),null=_A,blank=_A,default=0,editable=_B);hits=models.IntegerField(_('hits'),null=_A,blank=_A,default=0,editable=_B)
	def __str__(A):return f"{A.name}"
	def save(A,*B,**C):
		if not A.slug:A.slug=uuslug(A.name,instance=A,max_length=255)
		A.word_count=word_count(A.content);A.reading_time=reading_time(A.word_count);super().save(*B,**C)
...
class Banner(BaseAbstractModel):
	site=models.ForeignKey(Site,on_delete=models.CASCADE,verbose_name=_(_D));admin=models.ForeignKey(User,on_delete=models.PROTECT);photo=GenericRelation(Photo,verbose_name=_(_E));link=models.URLField(max_length=255,null=_A,blank=_A);position=models.PositiveIntegerField(choices=OptPosition.choices,default=OptPosition.DEFAULT);priority=models.SmallIntegerField(choices=OptPriority.choices,default=OptPriority.LOW,verbose_name=_(_W));status=models.SmallIntegerField(choices=OptStatusPublish.choices,default=OptStatusPublish.PUBLISHED)
	def __str__(A):
		B=''
		if A.priority==1:B='High'
		elif A.priority==2:B=_T
...
class Location(BaseAbstractModel,BaseGalleryModel,TranslatableModel):
	translations=TranslatedFields(title=encrypt(models.CharField(_(_C),max_length=LEN_TITLE)),subtitle=encrypt(models.CharField(_(_a),max_length=LEN_SUB_TITLE)));embed=CKEditor5Field(_('embed'),blank=_A,null=_A,config_name=_G);is_header_text=models.BooleanField(default=_B)
	def __str__(A):return f"{A.title}"
	def save(A,*B,**C):
		if not A.slug:A.slug=uuslug(A.title,instance=A,max_length=255)
		super().save(*B,**C)
...
class GoogleCalendar(BaseAbstractModel):
	...
	class Meta:verbose_name=_(_c);verbose_name_plural=_('google calendars')
	def __str__(A):return A.calendar_id
class GoogleCalendarDetail(BaseAbstractModel):
	site=models.ForeignKey(Site,on_delete=models.CASCADE,blank=_A,null=_A);google_calendar=models.ForeignKey(GoogleCalendar,on_delete=models.CASCADE,verbose_name=_(_c));event_id=models.CharField(_('google event ID'),max_length=LEN_NAME);start=models.DateTimeField(_('date start'),blank=_A,null=_A);end=models.DateTimeField(_('date end'),blank=_A,null=_A);summary=models.CharField(_('Summary'),max_length=LEN_TITLE,blank=_A,null=_A);description=models.TextField(_(_Y),blank=_A,null=_A);cal_name=models.TextField(_('calendar name'),blank=_A,null=_A);visibility=models.CharField(_('visibility'),max_length=LEN_SUB_TITLE,blank=_A,null=_A);location=models.CharField(_(_X),max_length=LEN_SUB_TITLE,blank=_A,null=_A);transparency=models.CharField(_('transparency'),max_length=LEN_SUB_TITLE,blank=_A,null=_A);cal_year=models.PositiveIntegerField(default=2023,blank=_A,editable=_B);cal_month=models.PositiveIntegerField(default=2023,blank=_A,editable=_B);cal_json=JSONField(null=_A,blank=_A)
...
class AutoPopular(models.Model):
	site=models.ForeignKey(Site,on_delete=models.CASCADE,verbose_name=_(_D));admin=models.ForeignKey(User,on_delete=models.PROTECT);title=models.CharField(_(_C),max_length=LEN_TITLE);sub_title=models.CharField(_(_F),max_length=LEN_SUB_TITLE,null=_A,blank=_A);slug=models.SlugField(max_length=255,default='',blank=_A,editable=_B);photo=GenericRelation(Photo,verbose_name=_(_E));categories=models.ForeignKey(Categories,on_delete=models.PROTECT,null=_A,blank=_A);kind=models.SmallIntegerField(choices=OptModelKinds.choices,verbose_name=_(_J));is_editable=models.BooleanField(default=_B);view_count=models.PositiveIntegerField(_(_U),default=1,editable=_B);created_at=models.DateTimeField(editable=_B);created_at_str=models.CharField(max_length=30,null=_A,blank=_A)
...
class AutoListPage(models.Model):
	site=models.ForeignKey(Site,on_delete=models.CASCADE,verbose_name=_(_D));admin=models.ForeignKey(User,on_delete=models.PROTECT);title=models.CharField(_(_C),max_length=LEN_TITLE);sub_title=models.CharField(_(_F),max_length=LEN_SUB_TITLE,null=_A,blank=_A);slug=models.SlugField(max_length=255,default='',blank=_A,editable=_B);photo=GenericRelation(Photo,verbose_name=_(_E));categories=models.ForeignKey(Categories,on_delete=models.PROTECT,null=_A,blank=_A);kind=models.SmallIntegerField(choices=OptModelKinds.choices,verbose_name=_(_J));is_editable=models.BooleanField(default=_B);created_at=models.DateTimeField(editable=_B);created_at_str=models.CharField(max_length=30,null=_A,blank=_A)
	class Meta:verbose_name=_(_h);verbose_name_plural=_(_h)
	def __str__(A):return f"{A.title}"
class DirectUpdate(BaseAbstractModel):
	site=models.ForeignKey(Site,on_delete=models.CASCADE);code=models.CharField(_('code'),max_length=LEN_NAME);value_type=models.SmallIntegerField(choices=OptValueType.choices,verbose_name=_('value_type'));value_text=models.CharField(_('text'),max_length=LEN_TITLE,null=_A,blank=_A);value_image=GenericRelation(Photo);value_textarea=CKEditor5Field(_('value textarea'),blank=_A,null=_A,config_name=_G)
	class Meta:verbose_name=_('direct update');verbose_name_plural=_('direct updates')
	def __str__(A):return f"{A.code}"
	...
	if A.file_path_doc:
		if os.path.isfile(A.file_path_doc.path):
			...
			if sender==GoogleCalendar:
				C=os.path.dirname(A.file_path_doc.path);D=_i;B=os.path.join(C,D)
	...
	if not A==E:
		if A:
			if os.path.isfile(A.path):
				os.remove(A.path)
				if B==GoogleCalendar:
					F=os.path.dirname(A.path);G=_i;D=os.path.join(F,G)
					if os.path.isfile(D):os.remove(D)
@receiver(post_delete,sender=Menu,dispatch_uid=_R)
@receiver(post_delete,sender=ModelList,dispatch_uid=_R)
@receiver(post_delete,sender=ModelListSetting,dispatch_uid=_R)
def menu_post_delete_handler(sender,**B):
	A=get_site_id(exposed_request)
	if A>0:print('clear cache delete',A);cache.delete(_j,version=A);cache.delete(_k,version=A);cache.delete(_l,version=A);cache.delete(_m,version=A);cache.delete(_n,version=A)
@receiver(post_save,sender=Menu,dispatch_uid=_S)
@receiver(post_save,sender=ModelList,dispatch_uid=_S)
@receiver(post_save,sender=ModelListSetting,dispatch_uid=_S)
def menu_post_save_handler(sender,**B):
	A=get_site_id(exposed_request)
	if A>0:print('clear cache update',A);cache.delete(_j,version=A);cache.delete(_k,version=A);cache.delete(_l,version=A);cache.delete(_m,version=A);cache.delete(_n,version=A)